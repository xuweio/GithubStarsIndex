# ⭐ GitHub Stars Index

> Updated: 2026-03-09 04:15 UTC · Total: 236

---

## 📑 Index
- [[#TunMax/openclaw_computer|TunMax/openclaw_computer]]
- [[#SuYxh/ai-news-aggregator|SuYxh/ai-news-aggregator]]
- [[#tw93/Mole|tw93/Mole]]
- [[#finvfamily/finshare|finvfamily/finshare]]
- [[#YoungCan-Wang/Wyckoff-Analysis|YoungCan-Wang/Wyckoff-Analysis]]
- [[#linuxhsj/openclaw-zero-token|linuxhsj/openclaw-zero-token]]
- [[#ckckck/UltimateSearchSkill|ckckck/UltimateSearchSkill]]
- [[#dongshuyan/CPA-Dashboard|dongshuyan/CPA-Dashboard]]
- [[#huggingface/skills|huggingface/skills]]
- [[#toby-bridges/epro-memory|toby-bridges/epro-memory]]
- [[#Kxiandaoyan/Memoh-v2|Kxiandaoyan/Memoh-v2]]
- [[#shuaiplus/nodewarden|shuaiplus/nodewarden]]
- [[#rikouu/memvault|rikouu/memvault]]
- [[#kuyermqi/tavily-proxy|kuyermqi/tavily-proxy]]
- [[#Pangu-Immortal/hunter-ai-content-factory|Pangu-Immortal/hunter-ai-content-factory]]
- [[#iamzifei/bookmark-is-learned|iamzifei/bookmark-is-learned]]
- [[#denda188/ClawIntelligentMemory|denda188/ClawIntelligentMemory]]
- [[#cheahjs/free-llm-api-resources|cheahjs/free-llm-api-resources]]
- [[#hesamsheikh/awesome-openclaw-usecases|hesamsheikh/awesome-openclaw-usecases]]
- [[#HKUDS/nanobot|HKUDS/nanobot]]
- [[#abingyyds/OpenAsst|abingyyds/OpenAsst]]
- [[#1186258278/OpenClawChineseTranslation|1186258278/OpenClawChineseTranslation]]
- [[#miaoxworld/OpenClawInstaller|miaoxworld/OpenClawInstaller]]
- [[#openclaw/openclaw|openclaw/openclaw]]
- [[#liangdabiao/Claude-Code-Stock-Deep-Research-Agent|liangdabiao/Claude-Code-Stock-Deep-Research-Agent]]
- [[#rookie-ricardo/erduo-skills|rookie-ricardo/erduo-skills]]
- [[#Open-Dev-Society/OpenStock|Open-Dev-Society/OpenStock]]
- [[#zstmfhy/zlibrary-to-notebooklm|zstmfhy/zlibrary-to-notebooklm]]
- [[#nguyenphutrong/quotio|nguyenphutrong/quotio]]
- [[#libukai/awesome-agent-skills|libukai/awesome-agent-skills]]
- [[#666ghj/MiroFish|666ghj/MiroFish]]
- [[#ZhuLinsen/daily_stock_analysis|ZhuLinsen/daily_stock_analysis]]
- [[#PleasePrompto/notebooklm-skill|PleasePrompto/notebooklm-skill]]
- [[#vbgate/learn-opencode|vbgate/learn-opencode]]
- [[#Fincept-Corporation/FinceptTerminal|Fincept-Corporation/FinceptTerminal]]
- [[#oyz8/ClawCloud-Run|oyz8/ClawCloud-Run]]
- [[#haierkeys/obsidian-fast-note-sync|haierkeys/obsidian-fast-note-sync]]
- [[#byJoey/cfnew|byJoey/cfnew]]
- [[#DigitalPlatDev/FreeDomain|DigitalPlatDev/FreeDomain]]
- [[#Ceelog/DictionaryByGPT4|Ceelog/DictionaryByGPT4]]
- [[#ClashConnectRules/Self-Configuration|ClashConnectRules/Self-Configuration]]
- [[#ink1ing/anti-api|ink1ing/anti-api]]
- [[#666OS/ClashMac|666OS/ClashMac]]
- [[#ebertti/awesome-telegram|ebertti/awesome-telegram]]
- [[#jaywcjlove/awesome-mac|jaywcjlove/awesome-mac]]
- [[#glidea/banana-prompt-quicker|glidea/banana-prompt-quicker]]
- [[#new-economy/aiagents-stock|new-economy/aiagents-stock]]
- [[#oficcejo/aiagents-stock|oficcejo/aiagents-stock]]
- [[#zijie0/HumanSystemOptimization|zijie0/HumanSystemOptimization]]
- [[#WCY-dt/MrRSS|WCY-dt/MrRSS]]
- [[#sese972010/CloudNav-|sese972010/CloudNav-]]
- [[#liuw1535/antigravity2api-nodejs|liuw1535/antigravity2api-nodejs]]
- [[#mtvpls/MoonTVPlus|mtvpls/MoonTVPlus]]
- [[#MetaCubeX/mihomo|MetaCubeX/mihomo]]
- [[#xiaoyaDev/xiaoya-alist|xiaoyaDev/xiaoya-alist]]
- [[#YYsuni/2025-blog-public|YYsuni/2025-blog-public]]
- [[#wuxingsanren/wildcat-vip-account|wuxingsanren/wildcat-vip-account]]
- [[#lu0b0/cloudflare-manager|lu0b0/cloudflare-manager]]
- [[#666ghj/BettaFish|666ghj/BettaFish]]
- [[#joname1/BestCFip|joname1/BestCFip]]
- [[#zskfree/News-Agent|zskfree/News-Agent]]
- [[#nashaofu/shell360|nashaofu/shell360]]
- [[#huppugo1/ProxyPoolWithUI|huppugo1/ProxyPoolWithUI]]
- [[#ginobefun/BestBlogs|ginobefun/BestBlogs]]
- [[#xincmm/sageread|xincmm/sageread]]
- [[#eooce/Appwrite|eooce/Appwrite]]
- [[#eooce/Auto-deploy-sap-and-keepalive|eooce/Auto-deploy-sap-and-keepalive]]
- [[#Atarity/deploy-your-own-saas|Atarity/deploy-your-own-saas]]
- [[#dongyubin/Free-AppleId-Serve|dongyubin/Free-AppleId-Serve]]
- [[#liyedanpdx/reddit-ai-trends|liyedanpdx/reddit-ai-trends]]
- [[#ptbsare/anx-calibre-manager|ptbsare/anx-calibre-manager]]
- [[#byJoey/Auto-deploy-sap-and-keepalive|byJoey/Auto-deploy-sap-and-keepalive]]
- [[#beck-8/subs-check|beck-8/subs-check]]
- [[#GMOogway/shadowrocket-rules|GMOogway/shadowrocket-rules]]
- [[#fish2018/pansou|fish2018/pansou]]
- [[#wzdnzd/aggregator|wzdnzd/aggregator]]
- [[#CherryHQ/hanlin-ai|CherryHQ/hanlin-ai]]
- [[#YoungLee-coder/SubMix|YoungLee-coder/SubMix]]
- [[#enescingoz/awesome-n8n-templates|enescingoz/awesome-n8n-templates]]
- [[#KroMiose/claude-code-nexus|KroMiose/claude-code-nexus]]
- [[#Zie619/n8n-workflows|Zie619/n8n-workflows]]
- [[#scriptscat/scriptcat|scriptscat/scriptcat]]
- [[#HuaYaoAI/FinGenius|HuaYaoAI/FinGenius]]
- [[#GakkiNoOne/hajimi-king|GakkiNoOne/hajimi-king]]
- [[#d60/twikit|d60/twikit]]
- [[#linshenkx/prompt-optimizer|linshenkx/prompt-optimizer]]
- [[#wassupjay/n8n-free-templates|wassupjay/n8n-free-templates]]
- [[#bepass-org/oblivion-desktop|bepass-org/oblivion-desktop]]
- [[#czlonkowski/n8n-mcp|czlonkowski/n8n-mcp]]
- [[#justlovemaki/AIClient-2-API|justlovemaki/AIClient-2-API]]
- [[#xiyewuqiu/new-lmage|xiyewuqiu/new-lmage]]
- [[#cfgranda/ps4ds|cfgranda/ps4ds]]
- [[#EcoPasteHub/EcoPaste|EcoPasteHub/EcoPaste]]
- [[#cf-pages/Telegraph-Image|cf-pages/Telegraph-Image]]
- [[#nicejade/wealth-tracker|nicejade/wealth-tracker]]
- [[#googleapis/genai-toolbox|googleapis/genai-toolbox]]
- [[#Shubhamsaboo/awesome-llm-apps|Shubhamsaboo/awesome-llm-apps]]
- [[#sansan0/TrendRadar|sansan0/TrendRadar]]
- [[#samanhappy/mcphub|samanhappy/mcphub]]
- [[#jbiaojerry/ebook-treasure-chest|jbiaojerry/ebook-treasure-chest]]
- [[#other-blowsnow/n8n-nodes-wechat-offiaccount|other-blowsnow/n8n-nodes-wechat-offiaccount]]
- [[#punkpeye/awesome-mcp-servers|punkpeye/awesome-mcp-servers]]
- [[#xxnuo/dns-benchmark|xxnuo/dns-benchmark]]
- [[#u14app/deep-research|u14app/deep-research]]
- [[#joeseesun/mcp-prompt-server|joeseesun/mcp-prompt-server]]
- [[#stackia/best-windows-apps|stackia/best-windows-apps]]
- [[#kanshurichard/enableAppleAI|kanshurichard/enableAppleAI]]
- [[#uxiaohan/ZYCS-IMG|uxiaohan/ZYCS-IMG]]
- [[#certimate-go/certimate|certimate-go/certimate]]
- [[#cmliu/SubsCheck-Win-GUI|cmliu/SubsCheck-Win-GUI]]
- [[#risin42/NagramX|risin42/NagramX]]
- [[#bestruirui/BestSub|bestruirui/BestSub]]
- [[#liyown/ai-trend-publish|liyown/ai-trend-publish]]
- [[#knowledgefxg/learning-english|knowledgefxg/learning-english]]
- [[#Anxcye/anx-reader|Anxcye/anx-reader]]
- [[#OpenHealthForAll/open-health|OpenHealthForAll/open-health]]
- [[#houjoe0829/shadowrocket-rules-joe|houjoe0829/shadowrocket-rules-joe]]
- [[#MarSeventh/CloudFlare-ImgBed|MarSeventh/CloudFlare-ImgBed]]
- [[#deepseek-ai/awesome-deepseek-integration|deepseek-ai/awesome-deepseek-integration]]
- [[#foamzou/melody|foamzou/melody]]
- [[#PDFMathTranslate/PDFMathTranslate|PDFMathTranslate/PDFMathTranslate]]
- [[#SteveSuv/remix-words-funny|SteveSuv/remix-words-funny]]
- [[#7Sageer/sublink-worker|7Sageer/sublink-worker]]
- [[#lijigang/write-prompt|lijigang/write-prompt]]
- [[#amxamxa/paywall-killer|amxamxa/paywall-killer]]
- [[#kingkare/GLaDOS|kingkare/GLaDOS]]
- [[#trungdq88/Awesome-Black-Friday-Cyber-Monday|trungdq88/Awesome-Black-Friday-Cyber-Monday]]
- [[#liujuntao123/chines-history-video|liujuntao123/chines-history-video]]
- [[#Reno9527/awesome-xiaobot|Reno9527/awesome-xiaobot]]
- [[#berryjam/mastering-bitcoin-3rd|berryjam/mastering-bitcoin-3rd]]
- [[#wechat-article/wechat-article-exporter|wechat-article/wechat-article-exporter]]
- [[#interaminense/learning-english|interaminense/learning-english]]
- [[#NeoVertex1/SuperPrompt|NeoVertex1/SuperPrompt]]
- [[#kenjihiranabe/The-Art-of-Linear-Algebra|kenjihiranabe/The-Art-of-Linear-Algebra]]
- [[#bookstairs/bookhunter|bookstairs/bookhunter]]
- [[#guobao2333/DeepLX-Serverless|guobao2333/DeepLX-Serverless]]
- [[#lanqian528/chat2api|lanqian528/chat2api]]
- [[#hiddify/hiddify-app|hiddify/hiddify-app]]
- [[#KotatsuApp/Kotatsu|KotatsuApp/Kotatsu]]
- [[#fatwang2/search2ai|fatwang2/search2ai]]
- [[#HqWu-HITCS/Awesome-Chinese-LLM|HqWu-HITCS/Awesome-Chinese-LLM]]
- [[#ChatGPTNextWeb/NextChat|ChatGPTNextWeb/NextChat]]
- [[#cmliu/WorkerVless2sub|cmliu/WorkerVless2sub]]
- [[#zhuima/awesome-cloudflare|zhuima/awesome-cloudflare]]
- [[#malinkang/weread2notion-pro|malinkang/weread2notion-pro]]
- [[#XIU2/CloudflareSpeedTest|XIU2/CloudflareSpeedTest]]
- [[#AttemptD/AfuseKt-release|AttemptD/AfuseKt-release]]
- [[#vvbbnn00/WARP-Clash-API|vvbbnn00/WARP-Clash-API]]
- [[#CrazyCreativeDream/Real-Coze-API|CrazyCreativeDream/Real-Coze-API]]
- [[#Kuingsmile/PicList|Kuingsmile/PicList]]
- [[#xlucn/oh-my-foss-android|xlucn/oh-my-foss-android]]
- [[#NodeBE4/impact|NodeBE4/impact]]
- [[#jeffyus/renwenjiangzuo|jeffyus/renwenjiangzuo]]
- [[#qianguyihao/blog-list|qianguyihao/blog-list]]
- [[#linexjlin/GPTs|linexjlin/GPTs]]
- [[#msterzhang/onelist|msterzhang/onelist]]
- [[#LetTTGACO/elog|LetTTGACO/elog]]
- [[#lobehub/lobehub|lobehub/lobehub]]
- [[#Xatta-Trone/medium-parser-extension|Xatta-Trone/medium-parser-extension]]
- [[#hiroi-sora/Umi-OCR|hiroi-sora/Umi-OCR]]
- [[#ninxsoft/Mist|ninxsoft/Mist]]
- [[#getActivity/AndroidIndex|getActivity/AndroidIndex]]
- [[#microsoft/Data-Science-For-Beginners|microsoft/Data-Science-For-Beginners]]
- [[#Loyalsoldier/clash-rules|Loyalsoldier/clash-rules]]
- [[#spiritLHLS/ecs|spiritLHLS/ecs]]
- [[#fanmingming/live|fanmingming/live]]
- [[#gaozhangmin/aliyunpan|gaozhangmin/aliyunpan]]
- [[#Mayandev/where-is-douban250|Mayandev/where-is-douban250]]
- [[#217heidai/adblockfilters|217heidai/adblockfilters]]
- [[#chatanywhere/GPT_API_free|chatanywhere/GPT_API_free]]
- [[#getsomecat/GetSomeCats|getsomecat/GetSomeCats]]
- [[#layou233/NeverIdle|layou233/NeverIdle]]
- [[#gps949/MirageSyno|gps949/MirageSyno]]
- [[#hehonghui/awesome-english-ebooks|hehonghui/awesome-english-ebooks]]
- [[#wdxtub/WDXpeak|wdxtub/WDXpeak]]
- [[#jxxghp/MoviePilot|jxxghp/MoviePilot]]
- [[#XPoet/picx|XPoet/picx]]
- [[#VPN-Subcription-Links/ClashX-V2Ray-TopFreeProxy|VPN-Subcription-Links/ClashX-V2Ray-TopFreeProxy]]
- [[#megaease/Remembering-Haoel|megaease/Remembering-Haoel]]
- [[#OdysseusYuan/LKY_OfficeTools|OdysseusYuan/LKY_OfficeTools]]
- [[#Johnshall/Shadowrocket-ADBlock-Rules-Forever|Johnshall/Shadowrocket-ADBlock-Rules-Forever]]
- [[#PlexPt/awesome-chatgpt-prompts-zh|PlexPt/awesome-chatgpt-prompts-zh]]
- [[#sub-store-org/Sub-Store|sub-store-org/Sub-Store]]
- [[#PKUFlyingPig/cs-self-learning|PKUFlyingPig/cs-self-learning]]
- [[#imDazui/Tvlist-awesome-m3u-m3u8|imDazui/Tvlist-awesome-m3u-m3u8]]
- [[#superleeyom/my-feed-OPML|superleeyom/my-feed-OPML]]
- [[#LampScript/qdaily_backup|LampScript/qdaily_backup]]
- [[#pt-plugins/PT-Plugin-Plus|pt-plugins/PT-Plugin-Plus]]
- [[#VirgilClyne/iRingo|VirgilClyne/iRingo]]
- [[#visiky/resume|visiky/resume]]
- [[#GrowingGit/GitHub-Chinese-Top-Charts|GrowingGit/GitHub-Chinese-Top-Charts]]
- [[#DottChen/WeChatRead_NoteConvertor|DottChen/WeChatRead_NoteConvertor]]
- [[#Paladinfeng/MiaoProject|Paladinfeng/MiaoProject]]
- [[#limbopro/Adblock4limbo|limbopro/Adblock4limbo]]
- [[#HarborLibrary/Political-Science|HarborLibrary/Political-Science]]
- [[#saveweb/rss-list|saveweb/rss-list]]
- [[#shibing624/pycorrector|shibing624/pycorrector]]
- [[#DIYgod/RSSHub|DIYgod/RSSHub]]
- [[#hepochen/FarBox|hepochen/FarBox]]
- [[#Loyalsoldier/v2ray-rules-dat|Loyalsoldier/v2ray-rules-dat]]
- [[#zhaoolee/garss|zhaoolee/garss]]
- [[#flomoapp/3rd-party-tools|flomoapp/3rd-party-tools]]
- [[#alaskasquirrel/Chinese-Podcasts|alaskasquirrel/Chinese-Podcasts]]
- [[#daliansky/OC-little|daliansky/OC-little]]
- [[#zlq4863947/proficient-tradingview|zlq4863947/proficient-tradingview]]
- [[#o1xhack/script|o1xhack/script]]
- [[#521xueweihan/HelloGitHub|521xueweihan/HelloGitHub]]
- [[#google/eleventy-high-performance-blog|google/eleventy-high-performance-blog]]
- [[#alirezamika/autoscraper|alirezamika/autoscraper]]
- [[#yihong0618/running_page|yihong0618/running_page]]
- [[#AboutRSS/ALL-about-RSS|AboutRSS/ALL-about-RSS]]
- [[#athena-xcy/CS50-Study-Group|athena-xcy/CS50-Study-Group]]
- [[#headllines/hackernews-daily|headllines/hackernews-daily]]
- [[#googlehosts/hosts|googlehosts/hosts]]
- [[#iwestlin/gdshare|iwestlin/gdshare]]
- [[#SuzukiHonoka/s905d-kernel-precompiled|SuzukiHonoka/s905d-kernel-precompiled]]
- [[#saturn-abhishek/awesome-github-profile-readme|saturn-abhishek/awesome-github-profile-readme]]
- [[#gdtool/gd-utils-docker|gdtool/gd-utils-docker]]
- [[#vitaminx/gd-utils|vitaminx/gd-utils]]
- [[#zvtvz/zvt|zvtvz/zvt]]
- [[#ivmm/Student-resources|ivmm/Student-resources]]
- [[#iptv-org/iptv|iptv-org/iptv]]
- [[#cgkings/script-store|cgkings/script-store]]
- [[#foxofice/sub_share|foxofice/sub_share]]
- [[#SunRelease/BaiduSVIP|SunRelease/BaiduSVIP]]
- [[#AlanDecode/Blog-With-GitHub-Boilerplate|AlanDecode/Blog-With-GitHub-Boilerplate]]
- [[#awesome-selfhosted/awesome-selfhosted|awesome-selfhosted/awesome-selfhosted]]
- [[#metowolf/vCards|metowolf/vCards]]
- [[#AktasC/Hackintosh-Elitebook-Broadwell|AktasC/Hackintosh-Elitebook-Broadwell]]
- [[#sqlsec/clover|sqlsec/clover]]
- [[#yisainan/web-interview|yisainan/web-interview]]
- [[#typlog/china-indie-podcasts|typlog/china-indie-podcasts]]
- [[#mewcoder/UsefulWebSites|mewcoder/UsefulWebSites]]
- [[#nailperry-zd/The-Economist|nailperry-zd/The-Economist]]
- [[#GitHubDaily/GitHubDaily|GitHubDaily/GitHubDaily]]
- [[#yuanxin69/m3u|yuanxin69/m3u]]

---

## TunMax/openclaw_computer

> [!info]
> ⭐ 41 · Dockerfile · 2026-03-09  
> [GitHub](https://github.com/TunMax/openclaw_computer)  
> `#openclaw` 
> 生成失败
> <sub>一个预装 OpenClaw 并具有桌面环境的 Linux 容器，适配 ModelScope、HuggingFace 等免费容器部署服务，通过浏览器即可畅玩体验 OpenClaw / A Docker container running Debian 12, the KDE desktop environment, and OpenClaw.</sub>

---
## SuYxh/ai-news-aggregator

> [!info]
> ⭐ 158 · TypeScript · 2026-03-09  
> [GitHub](https://github.com/SuYxh/ai-news-aggregator) · [Website](https://suyxh.github.io/ai-news-aggregator/)  
> `#ai-news-rss-aggregator` `#rss-reader` 
> 生成失败
> <sub>🤖 AI News Aggregator - 自动聚合 80+ AI/科技资讯源，支持 RSS 订阅导入，智能过滤 AI 相关内容，双语标题翻译，每 2 小时自动更新</sub>

---
## tw93/Mole

> [!info]
> ⭐ 38,776 · Shell · 2026-03-08  
> [GitHub](https://github.com/tw93/Mole) · [Website](https://x.com/HiTw93/status/2030252675829473336)  
> `#macOS 系统清理` `#磁盘优化` `#智能卸载` `#实时监控` `#analyzer` `#appcleaner` `#clean` `#cleaner` `#cleaner-cli` `#cleaner-script` `#command-line` `#daisydisk` `#istat` `#mac` `#macos` `#optimize` `#sensei` `#shell` `#uninstall` 
> Mole 是一款专为 macOS 设计的深度清理与系统优化工具，集 CleanMyMac、AppCleaner 等功能于一体。支持智能卸载应用及残留文件、可视化磁盘分析、实时系统监控，并提供缓存重建、Touch ID 配置等高级功能，帮助用户高效释放存储空间并提升系统性能。
> <sub>🐹 Deep clean and optimize your Mac.</sub>

---
## finvfamily/finshare

> [!info]
> ⭐ 122 · Python · 2026-03-09  
> [GitHub](https://github.com/finvfamily/finshare) · [Website](https://www.meepoquant.com)  
> `#china-stock` `#data-fetching` `#dataframe` `#eastmoney` `#finance` `#financial-data` `#pandas` `#python` `#stock` `#stock-market` `#trading` 
> 生成失败
> <sub>专业的金融数据获取工具库 - A Professional Financial Data Fetching Toolkit for   Python</sub>

---
## YoungCan-Wang/Wyckoff-Analysis

> [!info]
> ⭐ 157 · Python · 2026-03-09  
> [GitHub](https://github.com/YoungCan-Wang/Wyckoff-Analysis)  
> 
> 生成失败
> <sub>威科夫降临 分析A股，灵感来自 秋生trader @Hoyooyoo</sub>

---
## linuxhsj/openclaw-zero-token

> [!info]
> ⭐ 1,133 · TypeScript · 2026-03-08  
> [GitHub](https://github.com/linuxhsj/openclaw-zero-token)  
> 
> 生成失败
> <sub>OpenClaw: Use All Major AI Models NO API Token! Claude/ChatGPT/Gemini/DeepSeek/Doubao/Grok/Qwen/Manus/Kimi</sub>

---
## ckckck/UltimateSearchSkill

> [!info]
> ⭐ 147 · Shell · 2026-03-01  
> [GitHub](https://github.com/ckckck/UltimateSearchSkill)  
> 
> 生成失败
> <sub>Dual-engine web search Skill for OpenClaw/Pi: Grok AI + Tavily + FireCrawl</sub>

---
## dongshuyan/CPA-Dashboard

> [!info]
> ⭐ 281 · HTML · 2026-03-06  
> [GitHub](https://github.com/dongshuyan/CPA-Dashboard)  
> 
> 生成失败
> <sub>Dashboard For CLIProxyAPI</sub>

---
## huggingface/skills

> [!info]
> ⭐ 8,555 · Python · 2026-03-06  
> [GitHub](https://github.com/huggingface/skills)  
> 
> 生成失败

---
## toby-bridges/epro-memory

> [!info]
> ⭐ 67 · TypeScript · 2026-02-25  
> [GitHub](https://github.com/toby-bridges/epro-memory)  
> 
> 生成失败
> <sub>LLM-powered agent memory with 6-category classification and L0/L1/L2 tiered structure</sub>

---
## Kxiandaoyan/Memoh-v2

> [!info]
> ⭐ 325 · Go · 2026-03-06  
> [GitHub](https://github.com/Kxiandaoyan/Memoh-v2)  
> 
> 生成失败
> <sub>比OpenClaw强大数倍的人性化agent,龙虾有的我都有,龙虾没有的,我也有.</sub>

---
## shuaiplus/nodewarden

> [!info]
> ⭐ 1,083 · TypeScript · 2026-03-09  
> [GitHub](https://github.com/shuaiplus/nodewarden)  
> `#bitwarden` `#bitwarden-compatible` `#cloudflare` `#cloudflare-d1` `#cloudflare-r2` `#cloudflare-workers` `#end-to-end-encryption` `#nodewarden` `#password-manager` `#preact` `#self-hosted` `#serverless` `#typescript` `#vaultwarden` `#web-vault` `#workers` `#zero-knowledge` 
> 生成失败
> <sub>A third-party Bitwarden server running on Cloudflare Workers</sub>

---
## rikouu/memvault

> [!info]
> ⭐ 12 · TypeScript · 2026-02-19  
> [GitHub](https://github.com/rikouu/memvault)  
> 
> 生成失败
> <sub>MemVault  自托管的 AI 记忆管理平台 — 为 AI Agent 提供持久化的跨会话记忆能力。  基于 mem0 构建，提供完整的多租户管理界面、RESTful API、MCP 协议支持和 OpenClaw 插件，让你的 AI Agent 真正"记住"每一位用户。</sub>

---
## kuyermqi/tavily-proxy

> [!info]
> ⭐ 27 · TypeScript · 2026-01-23  
> [GitHub](https://github.com/kuyermqi/tavily-proxy)  
> 
> 生成失败
> <sub>Lightweight proxy for Tavily MCP</sub>

---
## Pangu-Immortal/hunter-ai-content-factory

> [!info]
> ⭐ 309 · Python · 2026-02-11  
> [GitHub](https://github.com/Pangu-Immortal/hunter-ai-content-factory) · [Website](https://github.com/Pangu-Immortal/hunter-ai-content-factory)  
> 
> 生成失败
> <sub>自动发布 24 小时情报系统：好文章的 80% 在于选题。Hunter AI 自动从 GitHub Trending、Twitter、HackerNews Reddit 等平台采集热点，用 AI 判断哪些值得写，自动生成文章和插图，自动发布。</sub>

---
## iamzifei/bookmark-is-learned

> [!info]
> ⭐ 336 · JavaScript · 2026-02-18  
> [GitHub](https://github.com/iamzifei/bookmark-is-learned)  
> 
> 生成失败
> <sub>收藏到就是学到，解决光收藏不看的问题。收藏的当下立刻学习或者丢弃</sub>

---
## denda188/ClawIntelligentMemory

> [!info]
> ⭐ 105 · N/A · 2026-03-05  
> [GitHub](https://github.com/denda188/ClawIntelligentMemory)  
> 
> 生成失败
> <sub>An OpenClaw intelligent memory system solution</sub>

---
## cheahjs/free-llm-api-resources

> [!info]
> ⭐ 14,608 · Python · 2026-03-08  
> [GitHub](https://github.com/cheahjs/free-llm-api-resources)  
> `#ai` `#claude` `#gemini` `#llama` `#llm` `#openai` 
> 生成失败
> <sub>A list of free LLM inference resources accessible via API.</sub>

---
## hesamsheikh/awesome-openclaw-usecases

> [!info]
> ⭐ 21,466 · N/A · 2026-03-04  
> [GitHub](https://github.com/hesamsheikh/awesome-openclaw-usecases)  
> `#awesome-list` `#clawdbot` `#moltbot` `#openclaw` `#openclaw-plugin` `#openclaw-setup` `#openclaw-skills` `#usecase` 
> 生成失败
> <sub>A community collection of OpenClaw use cases for making life easier.</sub>

---
## HKUDS/nanobot

> [!info]
> ⭐ 30,762 · Python · 2026-03-08  
> [GitHub](https://github.com/HKUDS/nanobot)  
> `#轻量级AI助手` `#多平台通信` `#MCP协议` `#模块化架构` 
> nanobot 是一款超轻量级个人 AI 助手，仅约 4000 行代码实现核心智能体功能，体积比同类项目小 99%。支持多平台通信（Slack、Discord、飞书等）、MCP 协议、多模态输入与记忆系统，具备高可靠性与模块化设计，适合快速部署与二次开发。
> <sub>"🐈 nanobot: The Ultra-Lightweight OpenClaw"</sub>

---
## abingyyds/OpenAsst

> [!info]
> ⭐ 89 · TypeScript · 2026-03-01  
> [GitHub](https://github.com/abingyyds/OpenAsst) · [Website](https://OpenAsst.Ai)  
> 
> 生成失败
> <sub>OpenAsst is an intelligent terminal assistant that lets you manage servers and execute system tasks using natural language. The project consists of two parts:  CLI Tool (Core) - A powerful command-line tool for AI-driven system operations Web Dashboard (Optional) - A visual interface for users who prefer GUI</sub>

---
## 1186258278/OpenClawChineseTranslation

> [!info]
> ⭐ 2,608 · JavaScript · 2026-03-09  
> [GitHub](https://github.com/1186258278/OpenClawChineseTranslation) · [Website](https://openclaw.qt.cool)  
> `#ai-agent` `#ai-assistant` `#chatbot` `#chinese` `#claude-ai` `#clawd` `#clawdbot` `#discord` `#llm` `#lobster` `#localization` `#moltbot` `#nodejs` `#open-source` `#openclaw` `#telegram` `#tool-calling` `#whatsapp` `#zh-cn` 
> 生成失败
> <sub>🦞 OpenClaw (Clawdbot/Moltbot) 汉化版 - 开源个人 AI 助手中文版 | Claude/ChatGPT LLM 接入 | WhatsApp/Telegram/Discord 多平台 | 每小时自动同步 | CLI + Dashboard 全中文 | 全流程搭建教程，以及排错指南！</sub>

---
## miaoxworld/OpenClawInstaller

> [!info]
> ⭐ 2,826 · Shell · 2026-02-03  
> [GitHub](https://github.com/miaoxworld/OpenClawInstaller)  
> `#clawdbot` `#moltbot` `#openclaw` 
> 生成失败
> <sub>ClawdBot 一键部署工具</sub>

---
## openclaw/openclaw

> [!info]
> ⭐ 282,898 · TypeScript · 2026-03-09  
> [GitHub](https://github.com/openclaw/openclaw) · [Website](https://openclaw.ai)  
> `#ai` `#assistant` `#crustacean` `#molty` `#openclaw` `#own-your-data` `#personal` 
> 生成失败
> <sub>Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞</sub>

---
## liangdabiao/Claude-Code-Stock-Deep-Research-Agent

> [!info]
> ⭐ 169 · N/A · 2026-03-07  
> [GitHub](https://github.com/liangdabiao/Claude-Code-Stock-Deep-Research-Agent) · [Website](https://linux.do/t/topic/1369037)  
> `#claude-code` 
> 生成失败
> <sub>本研究基于 Claude Code Deep Research 系统：  方法论: 8阶段股票投资尽调框架 智能体: 28个并行研究智能体 工具: WebSearch、WebFetch、综合分析 质量: 多空平衡、明确风险、数据验证。简单使用：/stock-research AAPL, I want a quick overview</sub>

---
## rookie-ricardo/erduo-skills

> [!info]
> ⭐ 358 · Python · 2026-02-02  
> [GitHub](https://github.com/rookie-ricardo/erduo-skills)  
> 
> 生成失败

---
## Open-Dev-Society/OpenStock

> [!info]
> ⭐ 9,323 · TypeScript · 2026-02-13  
> [GitHub](https://github.com/Open-Dev-Society/OpenStock) · [Website](https://openstock-ods.vercel.app)  
> `#coderabbit` `#inngest` `#nextjs` `#shadcn-ui` `#stock-market` `#tailwindcss` 
> 生成失败
> <sub>OpenStock is an open-source alternative to expensive market platforms. Track real-time prices, set personalized alerts, and explore detailed company insights — built openly, for everyone, forever free.</sub>

---
## zstmfhy/zlibrary-to-notebooklm

> [!info]
> ⭐ 1,419 · Python · 2026-01-17  
> [GitHub](https://github.com/zstmfhy/zlibrary-to-notebooklm)  
> 
> 生成失败
> <sub>一键将 Z-Library 书籍自动下载并上传到 Google NotebookLM</sub>

---
## nguyenphutrong/quotio

> [!info]
> ⭐ 3,911 · Swift · 2026-02-23  
> [GitHub](https://github.com/nguyenphutrong/quotio) · [Website](https://www.quotio.dev)  
> `#ai-tools` `#developer-tools` `#proxy` `#quota-monitor` 
> 生成失败
> <sub>Stop juggling AI accounts. Quotio is a beautiful native macOS menu bar app that unifies your Claude, Gemini, OpenAI, Qwen, and Antigravity subscriptions – with real-time quota tracking and smart auto-failover for AI coding tools like Claude Code, OpenCode, and Droid.</sub>

---
## libukai/awesome-agent-skills

> [!info]
> ⭐ 2,682 · Python · 2026-03-06  
> [GitHub](https://github.com/libukai/awesome-agent-skills) · [Website](https://x.com/libukai)  
> `#agent` `#agentskills` `#ai-agents` `#anthropic` `#awesome-list` `#claude-code` `#codex` `#gemini` `#opencode` 
> 生成失败
> <sub>Agent Skills 终极指南：快速入门、推荐技能、最新资讯与实战案例 ｜The Definitive Guide to Agent Skills: Quick Start, Recommended Skills, Latest News, and Practical Case Studies</sub>

---
## 666ghj/MiroFish

> [!info]
> ⭐ 7,560 · Python · 2026-03-07  
> [GitHub](https://github.com/666ghj/MiroFish) · [Website](https://666ghj.github.io/mirofish-demo/)  
> `#agent-memory` `#financial-forecasting` `#future-prediction` `#knowledge-graph` `#llms` `#multi-agent-simulation` `#public-opinion-analysis` `#python3` `#social-prediction` `#swarm-intelligence` 
> 生成失败
> <sub>A Simple and Universal Swarm Intelligence Engine, Predicting Anything. 简洁通用的群体智能引擎，预测万物</sub>

---
## ZhuLinsen/daily_stock_analysis

> [!info]
> ⭐ 17,614 · Python · 2026-03-08  
> [GitHub](https://github.com/ZhuLinsen/daily_stock_analysis)  
> `#agent` `#ai` `#aigc` `#gemini` `#llm` `#quant` `#quantitative-trading` `#rag` `#stock` 
> 生成失败
> <sub>LLM驱动的 A/H/美股智能分析器，多数据源行情 + 实时新闻 + Gemini 决策仪表盘 + 多渠道推送，零成本，纯白嫖，定时运行</sub>

---
## PleasePrompto/notebooklm-skill

> [!info]
> ⭐ 4,262 · Python · 2025-11-21  
> [GitHub](https://github.com/PleasePrompto/notebooklm-skill)  
> 
> 生成失败
> <sub>Use this skill to enable Claude Code to communicate directly with your Google NotebookLM notebooks. Query your uploaded documents and get source-grounded, citation-backed answers from Gemini. Features browser automation, library management, persistent authentication, and answers exclusively from your own knowledge base.</sub>

---
## vbgate/learn-opencode

> [!info]
> ⭐ 1,027 · Shell · 2026-03-06  
> [GitHub](https://github.com/vbgate/learn-opencode) · [Website](https://learnopencode.com/)  
> `#opencode` `#opencode-ai` `#opencode-cluster` `#opencode-plugin` `#opencode-plugins` 
> 生成失败
> <sub>OpenCode 中文实战课源码与内容仓库：一课一页，覆盖入门到实战工作流。</sub>

---
## Fincept-Corporation/FinceptTerminal

> [!info]
> ⭐ 2,759 · TypeScript · 2026-03-05  
> [GitHub](https://github.com/Fincept-Corporation/FinceptTerminal) · [Website](https://fincept.in)  
> `#bloomberg-terminal` `#contributions-welcome` `#finance` `#financial-markets` `#foss` `#good-first-issue` `#help-wanted` `#investing` `#investment` `#investment-research` `#machine-learning` `#opensource` `#python` `#quantitative-finance` `#stock-market` `#stocks` 
> 生成失败
> <sub>FinceptTerminal is a modern finance application offering advanced market analytics, investment research, and economic data tools, designed for interactive exploration and data-driven decision-making in a user-friendly environment.</sub>

---
## oyz8/ClawCloud-Run

> [!info]
> ⭐ 259 · Python · 2026-02-09  
> [GitHub](https://github.com/oyz8/ClawCloud-Run)  
> 
> 生成失败
> <sub>自动登录 ClawCloud，保持账户活跃，支持设备验证 + 两步验证</sub>

---
## haierkeys/obsidian-fast-note-sync

> [!info]
> ⭐ 1,228 · TypeScript · 2026-03-07  
> [GitHub](https://github.com/haierkeys/obsidian-fast-note-sync)  
> `#better-sync` `#obsidian` `#obsidian-note-sync` 
> 生成失败
> <sub>Can be privately deployed, focusing on providing Obsidian users with a seamless, distraction-free note synchronization plugin with real-time sync across multiple platforms, supporting Mac, Windows, Android, iOS, and offering multilingual support.可私有化部署，专注为 Obsidian 用户提供无打扰、丝般顺滑、多端实时同步的多平台笔记同步插件。</sub>

---
## byJoey/cfnew

> [!info]
> ⭐ 10,066 · N/A · 2026-03-06  
> [GitHub](https://github.com/byJoey/cfnew)  
> 
> 生成失败

---
## DigitalPlatDev/FreeDomain

> [!info]
> ⭐ 151,400 · HTML · 2026-03-02  
> [GitHub](https://github.com/DigitalPlatDev/FreeDomain) · [Website](https://domain.digitalplat.org)  
> `#digitalplat` `#domain` `#domain-platform` `#domainname` `#domains` `#free` `#freedomain` 
> 生成失败
> <sub>DigitalPlat FreeDomain: Free Domain For Everyone</sub>

---
## Ceelog/DictionaryByGPT4

> [!info]
> ⭐ 5,745 · HTML · 2026-02-27  
> [GitHub](https://github.com/Ceelog/DictionaryByGPT4) · [Website](https://ceelog.github.io/DictionaryByGPT4/)  
> `#gpt-4` `#gpt4` 
> 生成失败
> <sub>一本 GPT4 生成的单词书📚，超过 8000 个单词分析，涵盖了词义、例句、词根词缀、变形、文化背景、记忆技巧和小故事</sub>

---
## ClashConnectRules/Self-Configuration

> [!info]
> ⭐ 1,175 · N/A · 2026-01-16  
> [GitHub](https://github.com/ClashConnectRules/Self-Configuration)  
> 
> 生成失败

---
## ink1ing/anti-api

> [!info]
> ⭐ 311 · TypeScript · 2026-03-02  
> [GitHub](https://github.com/ink1ing/anti-api) · [Website](http://localhost:8964/quota)  
> 
> 生成失败
> <sub>Turn Antigravity / codex / github copilot into Anthropic & Openai API compatible server. Usable with Claude Code / Xcode etc.</sub>

---
## 666OS/ClashMac

> [!info]
> ⭐ 4,813 · N/A · 2026-01-31  
> [GitHub](https://github.com/666OS/ClashMac) · [Website](https://clashmac.app)  
> `#macOS 代理` `#SwiftUI` `#流量可视化` `#TUN 模式` `#clash` `#clash-meta` `#macos` `#mihomo` `#network` `#proxy` `#swiftui` 
> ClashMac 是一款专为 macOS 打造的原生代理工具，基于 SwiftUI 构建，深度集成系统 API。提供可视化路由地图、连接拓扑图与流量统计，支持系统代理与 TUN 模式，具备隐私保护、实时测速与全局快捷键等功能，带来直观流畅的代理体验。
> <sub>Native Proxy Experience Built for macOS</sub>

---
## ebertti/awesome-telegram

> [!info]
> ⭐ 4,528 · N/A · 2026-02-19  
> [GitHub](https://github.com/ebertti/awesome-telegram)  
> `#awesome-list` `#telegram` 
> 生成失败
> <sub>Collection great groups, channels, bots and libraries for Telegram</sub>

---
## jaywcjlove/awesome-mac

> [!info]
> ⭐ 99,712 · JavaScript · 2026-03-08  
> [GitHub](https://github.com/jaywcjlove/awesome-mac) · [Website](https://git.io/macx)  
> `#macOS 软件` `#精选列表` `#开发工具` `#效率应用` `#app` `#apple` `#application` `#apps` `#awesome` `#awesome-list` `#awesome-lists` `#awesome-mac` `#desktop-app` `#desktop-application` `#desktop-apps` `#list` `#mac` `#mac-osx` `#macos` `#macos-app` `#macos-apps` `#macosx` `#software` 
> 该项目是一个精选的 macOS 优质软件集合，按类别整理，涵盖开发工具、效率应用、设计工具等。支持多语言文档，标注开源、免费及 App Store 链接，方便用户快速发现高质量 Mac 应用。
> <sub> Now we have become very big, Different from the original idea. Collect premium software in various categories.</sub>

---
## glidea/banana-prompt-quicker

> [!info]
> ⭐ 2,229 · JavaScript · 2026-03-09  
> [GitHub](https://github.com/glidea/banana-prompt-quicker) · [Website](https://glidea.github.io/banana-prompt-quicker/)  
> `#banana` `#gemini` `#prompt` 
> 生成失败
> <sub>🍌Awesome Prompts; Nano Banana；Banana Pro; Gemini；AI Studio；Prompt Quickly  [网页监控工具-预定送会员：https://waitlist.dingding.glidea.app]</sub>

---
## new-economy/aiagents-stock

> [!info]
> ⭐ 1 · N/A · 2025-10-07  
> [GitHub](https://github.com/new-economy/aiagents-stock) · [Website](http://192.227.137.234:8501)  
> 
> 生成失败
> <sub>复合多AI智能体股票团队分析监测系统，基于多个ai智能体，模拟证券分析师团队分析过程，提供全方位的股票投资分析和决策建议，并支持实时监测关键点位，发送警报信息，预留miniqmt接口，支持量化交易，真正做到了ai帮你来炒股，适合大陆A股。下方测试网站请填入deepseek临时key，测试完请删除。测试服务器在美东，配置低，延迟大，请谅解</sub>

---
## oficcejo/aiagents-stock

> [!info]
> ⭐ 928 · Python · 2026-03-08  
> [GitHub](https://github.com/oficcejo/aiagents-stock) · [Website](https://okx-zh.zhongdu.net/)  
> 
> 生成失败
> <sub>复合多AI智能体股票团队分析盯盘系统，基于多个ai智能体，模拟证券分析师团队分析过程，提供全方位的股票投资分析和决策建议，新增游资龙虎榜跟踪分析、板块预警轮动分析，支持批量多线程分析，支持实时监测关键点位，发送警报信息，预留miniqmt接口，支持量化交易，真正做到了ai帮你来炒股，适合大陆A股。下方测试网站请填入deepseek临时key，测试完请删除，否则有泄露风险。注：测试云服务访问过多，数据源有可能会拒绝，如需使用，请部署本地。链接是eth超短线项目，喜欢玩币的可以试试，比较稳</sub>

---
## zijie0/HumanSystemOptimization

> [!info]
> ⭐ 21,601 · N/A · 2025-09-10  
> [GitHub](https://github.com/zijie0/HumanSystemOptimization)  
> 
> 生成失败
> <sub>健康学习到150岁 - 人体系统调优不完全指南</sub>

---
## WCY-dt/MrRSS

> [!info]
> ⭐ 1,881 · Go · 2026-03-07  
> [GitHub](https://github.com/WCY-dt/MrRSS) · [Website](https://mrrss.ch3nyang.top/)  
> `#ai` `#atom` `#feed-reader` `#feeds` `#rss` `#rss-reader` 
> 生成失败
> <sub>A modern, cross-platform, and free AI RSS reader. 一个现代化、跨平台且免费的 AI RSS 阅读器.</sub>

---
## sese972010/CloudNav-

> [!info]
> ⭐ 221 · TypeScript · 2025-12-17  
> [GitHub](https://github.com/sese972010/CloudNav-)  
> 
> 生成失败
> <sub>托管在cloudflare上的轻量化导航，可以同步谷歌书签</sub>

---
## liuw1535/antigravity2api-nodejs

> [!info]
> ⭐ 716 · JavaScript · 2026-03-07  
> [GitHub](https://github.com/liuw1535/antigravity2api-nodejs)  
> 
> 生成失败

---
## mtvpls/MoonTVPlus

> [!info]
> ⭐ 1,388 · TypeScript · 2026-03-04  
> [GitHub](https://github.com/mtvpls/MoonTVPlus)  
> 
> 生成失败
> <sub>MoonTVPlus 是基于 MoonTV v100 二次开发的增强版影视聚合播放器。它在原版基础上新增了外部播放器支持、视频超分、弹幕系统、评论抓取等实用功能，提供更强大的观影体验。</sub>

---
## MetaCubeX/mihomo

> [!info]
> ⭐ 27,480 · Python · 2026-03-08  
> [GitHub](https://github.com/MetaCubeX/mihomo) · [Website](https://wiki.metacubex.one)  
> `#Pydantic` `#API 解析` `#崩坏星穹铁道` `#数据模型` `#honkai-star-rail` `#mihomo` `#python` `#star-rail-api` 
> mihomo 是一个基于 Pydantic 的 Python 库，用于解析《崩坏：星穹铁道》的 Mihomo API 数据，提供类型提示与自动补全支持。支持 V1/V2 两种数据格式，包含角色、成就、头像等结构化信息，并提供去重、合并、持久化等实用工具。
> <sub>A simple Python Pydantic model for Honkai: Star Rail parsed data from the Mihomo API.</sub>

---
## xiaoyaDev/xiaoya-alist

> [!info]
> ⭐ 8,070 · Shell · 2026-03-08  
> [GitHub](https://github.com/xiaoyaDev/xiaoya-alist)  
> 
> 生成失败
> <sub>小雅Alist的相关周边</sub>

---
## YYsuni/2025-blog-public

> [!info]
> ⭐ 1,294 · TypeScript · 2026-03-06  
> [GitHub](https://github.com/YYsuni/2025-blog-public) · [Website](https://www.yysuni.com/)  
> `#blog` `#github-app` 
> 生成失败
> <sub>开源blog，让你不写代码就开始写 blog，管理 blog 内容。无需服务器，无需费用。</sub>

---
## wuxingsanren/wildcat-vip-account

> [!info]
> ⭐ 8,478 · N/A · 2025-07-19  
> [GitHub](https://github.com/wuxingsanren/wildcat-vip-account) · [Website](http://yemao.one)  
> 
> 生成失败
> <sub>野猫 - 每天分享最新的百度网盘SVIP、迅雷超级会员、手机话费折扣充值、霸王餐免费吃VIP（美团、饿了么、大众点评、肯德基、麦当劳、星巴克）、饿了么超级会员、美团外卖会员&红包券、爱奇艺VIP会员、腾讯视频VIP、优酷VIP会员、哔哩哔哩大会员、百度文库VIP、QQ音乐VIP、网易云黑胶VIP、喜马拉雅VIP、樊登读书会VIP、千图网VIP、包图网VIP、摄图网VIP、CSDN下载VIP、天眼查VIP、苹果ID等等各类VIP帐号，随取随用，完全免费，绝无套路，同时提供：百度文库VIP下载、图库素材VIP下载、学术文献VIP下载（知网维普万方读秀龙源超星、英文数据库、法律数据库、医学数据库、金融数据库）、全网视频VIP解析、全网音乐MP3免费听及下载、微信域名拦截检测API ，欢迎推荐分享给</sub>

---
## lu0b0/cloudflare-manager

> [!info]
> ⭐ 163 · TypeScript · 2025-12-30  
> [GitHub](https://github.com/lu0b0/cloudflare-manager)  
> 
> 生成失败
> <sub>一个强大的Cloudflare多账号Workers批量管理系统，支持账号管理、批量操作、实时监控和Workers脚本管理。</sub>

---
## 666ghj/BettaFish

> [!info]
> ⭐ 36,848 · Python · 2026-03-05  
> [GitHub](https://github.com/666ghj/BettaFish) · [Website](https://deepwiki.com/666ghj/BettaFish)  
> `#多智能体系统` `#舆情分析` `#多模态处理` `#自主数据采集` `#agent-framework` `#data-analysis` `#deep-research` `#deep-search` `#llms` `#multi-agent-system` `#nlp` `#public-opinion-analysis` `#python3` `#sentiment-analysis` 
> 微舆（BettaFish）是一个从零构建的多智能体舆情分析系统，支持全自动采集国内外30+社媒平台数据，融合微调模型与统计模型，通过Agent“论坛”协作机制实现深度舆情还原与趋势预测。具备多模态解析、公私域数据融合及轻量化部署能力，助力打破信息茧房，辅助智能决策。
> <sub>微舆：人人可用的多Agent舆情分析助手，打破信息茧房，还原舆情原貌，预测未来走向，辅助决策！从0实现，不依赖任何框架。</sub>

---
## joname1/BestCFip

> [!info]
> ⭐ 71 · Python · 2026-03-09  
> [GitHub](https://github.com/joname1/BestCFip)  
> 
> 生成失败
> <sub>自动采集Cloudflare优选IP，Workers、Pages自定义优选ip专用URL</sub>

---
## zskfree/News-Agent

> [!info]
> ⭐ 94 · Python · 2026-03-08  
> [GitHub](https://github.com/zskfree/News-Agent) · [Website](http://www.zskksz.asia/News-Agent/)  
> 
> 生成失败
> <sub>新闻聚合Agent</sub>

---
## nashaofu/shell360

> [!info]
> ⭐ 1,078 · TypeScript · 2026-03-09  
> [GitHub](https://github.com/nashaofu/shell360) · [Website](https://nashaofu.github.io/shell360/)  
> `#cross-platform` `#rust` `#sftp` `#ssh` `#ssh-client` `#tauri` `#tauri-app` 
> 生成失败
> <sub>Shell360 is a cross-platform SSH and SFTP client.</sub>

---
## huppugo1/ProxyPoolWithUI

> [!info]
> ⭐ 314 · Vue · 2025-10-20  
> [GitHub](https://github.com/huppugo1/ProxyPoolWithUI)  
> 
> 生成失败
> <sub>核心特性 :bullseye: 自动爬取 - 从 14+ 个免费代理源自动爬取代理  :white_check_mark: 自动验证 - 实时验证代理可用性，只返回可用代理  :globe_with_meridians: 多协议支持 - HTTP、HTTPS、SOCKS4、SOCKS5  :artist_palette: 现代化 UI - 基于 Nuxt 3 + Vue 3 + Ant Design Vue  :high_voltage: Clash 订阅 - 一键导入 Clash，支持 50+ 国家节点筛选  :counterclockwise_arrows_button: 实时监控 - 代理状态、爬取器状态实时展示  :round_pushpin: 地理位置 - 自动识别代理 IP 归</sub>

---
## ginobefun/BestBlogs

> [!info]
> ⭐ 3,104 · N/A · 2026-03-07  
> [GitHub](https://github.com/ginobefun/BestBlogs) · [Website](https://bestblogs.dev)  
> `#ai` `#business` `#product` `#programming` 
> 生成失败
> <sub>bestblogs.dev - 汇集顶级编程、人工智能、产品、科技文章，大语言模型摘要评分辅助阅读，探索编程和技术未来</sub>

---
## xincmm/sageread

> [!info]
> ⭐ 699 · TypeScript · 2025-10-11  
> [GitHub](https://github.com/xincmm/sageread)  
> 
> 生成失败
> <sub>一款 AI 辅助阅读器，三栏设计实现笔记、阅读、对话同屏交互，让深度阅读更高效</sub>

---
## eooce/Appwrite

> [!info]
> ⭐ 636 · N/A · 2025-10-11  
> [GitHub](https://github.com/eooce/Appwrite)  
> 
> 生成失败

---
## eooce/Auto-deploy-sap-and-keepalive

> [!info]
> ⭐ 705 · JavaScript · 2026-02-14  
> [GitHub](https://github.com/eooce/Auto-deploy-sap-and-keepalive)  
> `#sap` `#trojan` `#vless-ws-tls` `#vmess` `#vmess-ws-tls` 
> 生成失败
> <sub>Auto deploy sap and keepalive</sub>

---
## Atarity/deploy-your-own-saas

> [!info]
> ⭐ 8,258 · N/A · 2025-12-16  
> [GitHub](https://github.com/Atarity/deploy-your-own-saas)  
> `#awesome` `#awesome-list` `#list` `#open-source` `#self-hosted` 
> 生成失败
> <sub>List of "only yours" cloud services for everyday needs :black_flag:</sub>

---
## dongyubin/Free-AppleId-Serve

> [!info]
> ⭐ 2,703 · Python · 2026-02-14  
> [GitHub](https://github.com/dongyubin/Free-AppleId-Serve) · [Website](https://help.wwkejishe.top/free-shadowrocket)  
> `#android` `#appleid` `#chatgpt` `#clash` `#clashx` `#clashx-pro` `#gfw` `#openai` `#quantumult-x` `#shadowsocks` `#shadowsocksr` `#ssr` `#stash` `#tizi` `#v2ray` 
> 生成失败
> <sub>2026年3月更新🚀苹果美区账号 APPLE ID🚀免费共享Shadowrocket（小火箭）账号,Quantumult X，圈x，stash，小火箭节点，🚀 免费订阅地址，🚀 免费节点，🚀 每天更新一次，共享节点，节点质量高可用，完全免费。免费clash订阅地址，免费翻墙，免费科学上网，免费梯子，免费ss/v2ray/trojan/clash节点,翻墙梯子，Openai ChatGPT,代理,proxy代理科学上网,TG代理,电报代理,Telegram代理,免费共享苹果外区ID,clashx Pro客户端下载,Free subscription address,使用教程</sub>

---
## liyedanpdx/reddit-ai-trends

> [!info]
> ⭐ 804 · Python · 2026-03-08  
> [GitHub](https://github.com/liyedanpdx/reddit-ai-trends)  
> `#ai` `#deepseek-r1` `#ranking` `#reports` `#trend` 
> 生成失败
> <sub>Stay ahead of AI trends with automated Reddit insights! 🚀 This tool scans AI-related Reddit communities in English & Chinese, using Reddit Official API, DeepSeek R1 by OpenRouter to analyze posts, summarize key discussions, and track trends. Daily rankings hot topics—catch emerging trends before they go mainstream! (Updated every 6 AM CDT)</sub>

---
## ptbsare/anx-calibre-manager

> [!info]
> ⭐ 166 · Python · 2025-11-23  
> [GitHub](https://github.com/ptbsare/anx-calibre-manager)  
> `#anx-reader` `#audiobook-manager` `#audiobook-player` `#audiobooks` `#calibre` `#docker` `#ebook` `#ebookmanager` `#flask` `#pwa` `#web` 
> 生成失败
> <sub>A modern, mobile-first web application to manage your ebook library, integrating with Calibre and providing a personal WebDAV server for your Anx-reader compatible devices.</sub>

---
## byJoey/Auto-deploy-sap-and-keepalive

> [!info]
> ⭐ 353 · N/A · 2025-09-22  
> [GitHub](https://github.com/byJoey/Auto-deploy-sap-and-keepalive)  
> 
> 生成失败
> <sub>Auto deploy sap and keepalive</sub>

---
## beck-8/subs-check

> [!info]
> ⭐ 4,316 · Go · 2026-03-02  
> [GitHub](https://github.com/beck-8/subs-check) · [Website](https://t.me/subs_check)  
> `#clash` `#mihomo` `#v2ray` 
> 生成失败
> <sub>订阅转换、测速、测活、流媒体检测、重命名、导出为任意格式订阅的工具</sub>

---
## GMOogway/shadowrocket-rules

> [!info]
> ⭐ 4,136 · N/A · 2026-03-08  
> [GitHub](https://github.com/GMOogway/shadowrocket-rules)  
> `#adblock` `#direct` `#gfw` `#gfwlist` `#ios` `#module` `#proxy` `#reject` `#rules` `#shadowrocket` `#ss` `#v2ray` `#xray` 
> 生成失败
> <sub>小火箭规则🚀，小火箭模块，小火箭配置，shadowrocket规则，shadowrocket rules，最全面的直连（DIRECT）、代理（PROXY）、屏蔽（REJECT）规则，自动构建，每日更新</sub>

---
## fish2018/pansou

> [!info]
> ⭐ 12,081 · Go · 2025-12-13  
> [GitHub](https://github.com/fish2018/pansou)  
> 
> 生成失败
> <sub>PanSou是一款高性能的网盘资源搜索API服务，支持TG频道和插件搜索。系统设计以性能和可扩展性为核心，支持多频道多插件并发搜索、结果智能排序和网盘类型分类。docker集成前后端，一键启动，开箱即用。 https://so.252035.xyz/</sub>

---
## wzdnzd/aggregator

> [!info]
> ⭐ 6,330 · Python · 2026-03-09  
> [GitHub](https://github.com/wzdnzd/aggregator) · [Website](https://github.com/wzdnzd/aggregator)  
> `#代理池` `#爬虫聚合` `#Clash/V2Ray转换` `#proxypool` 
> 一个功能强大的免费代理池构建工具，支持从 Telegram、GitHub、Google 等多平台爬取代理资源，自动验证活性并转换为 Clash、V2Ray、SingBox 等主流格式。提供 process.py 完整处理与 collect.py 快速收集两种模式，支持 GitHub Gist 等多种存储后端，适用于搭建个人或共享代理订阅服务。
> <sub>One-stop Proxies Crawling and Aggregation Platform</sub>

---
## CherryHQ/hanlin-ai

> [!info]
> ⭐ 210 · Swift · 2026-01-24  
> [GitHub](https://github.com/CherryHQ/hanlin-ai)  
> 
> 生成失败
> <sub>A SwiftUI-based Chat App for LLMs on iOS</sub>

---
## YoungLee-coder/SubMix

> [!info]
> ⭐ 66 · TypeScript · 2026-03-09  
> [GitHub](https://github.com/YoungLee-coder/SubMix) · [Website](https://subm1x.vercel.app)  
> `#clash-meta` `#mihomo` `#nextjs` `#shadowsocks` `#trojan` `#vless` 
> 生成失败
> <sub>企业网络协议转换器，将单独的协议订阅链接转换为 Mihomo 内核 YAML 配置文件</sub>

---
## enescingoz/awesome-n8n-templates

> [!info]
> ⭐ 19,916 · N/A · 2025-11-02  
> [GitHub](https://github.com/enescingoz/awesome-n8n-templates) · [Website](https://n8n.partnerlinks.io/h1pwwf5m4toe)  
> `#automation` `#automation-templates` `#integration` `#n8n` `#n8n-automation` `#n8n-template` `#no-code-ai` `#no-code-automation` `#workflow-automation` 
> 生成失败
> <sub>Supercharge your workflow automation with this curated collection of n8n templates! Instantly connect your favorite apps-like Gmail, Telegram, Google Drive, Slack, and more-with ready-to-use, AI-powered automations. Save time, boost productivity, and unlock the true potential of n8n in just a few clicks.</sub>

---
## KroMiose/claude-code-nexus

> [!info]
> ⭐ 235 · TypeScript · 2025-08-20  
> [GitHub](https://github.com/KroMiose/claude-code-nexus) · [Website](https://claude.nekro.ai/)  
> `#claude` `#claude-code` `#cloudflare` `#oneapi` `#openai` `#worker` 
> 生成失败
> <sub>Claude API Proxy platform - Seamlessly forward Claude Code requests to any OpenAI-compatible API service with smart model mapping, streaming support, deployed on Cloudflare Worker.</sub>

---
## Zie619/n8n-workflows

> [!info]
> ⭐ 52,693 · Python · 2026-02-11  
> [GitHub](https://github.com/Zie619/n8n-workflows)  
> `#n8n工作流` `#AI安全扫描` `#自动化集成` `#合规审计` 
> 该仓库汇集了4343+个n8n自动化工作流，涵盖365+种集成，是n8n生态中最全面的公开工作流集合。同时引入AI-BOM工具，首次实现对n8n工作流中AI安全风险（如硬编码密钥、未认证AI代理、MCP客户端等）的自动化扫描，支持生成合规报告，助力满足欧盟AI法案要求。
> <sub>all of the workflows of n8n i could find (also from the site itself)</sub>

---
## scriptscat/scriptcat

> [!info]
> ⭐ 4,147 · TypeScript · 2026-03-07  
> [GitHub](https://github.com/scriptscat/scriptcat) · [Website](https://docs.scriptcat.org)  
> `#chrome-extension` `#greasemonkey-userscript` `#scriptcat` `#userscript` `#webextension` 
> 生成失败
> <sub>ScriptCat, a browser extension that can execute userscript; 脚本猫，一个可以执行用户脚本的浏览器扩展</sub>

---
## HuaYaoAI/FinGenius

> [!info]
> ⭐ 2,610 · Python · 2025-08-06  
> [GitHub](https://github.com/HuaYaoAI/FinGenius)  
> 
> 生成失败

---
## GakkiNoOne/hajimi-king

> [!info]
> ⭐ 907 · Python · 2025-08-09  
> [GitHub](https://github.com/GakkiNoOne/hajimi-king)  
> 
> 生成失败
> <sub>人人都是哈基米大王</sub>

---
## d60/twikit

> [!info]
> ⭐ 4,100 · Python · 2025-07-06  
> [GitHub](https://github.com/d60/twikit) · [Website](https://twikit.readthedocs.io/en/latest/twikit.html)  
> `#bot` `#client` `#python` `#python-web-scraper` `#python3` `#scrape` `#scraper` `#scraping` `#search` `#tweepy` `#twitter` `#twitter-api` `#twitter-bot` `#twitter-client` `#twitter-internal-api` `#twitter-scraper` `#wrapper` `#x` `#x-api` 
> 生成失败
> <sub>Twitter API Scraper | Without an API key | Twitter Internal API | Free | Twitter scraper | Twitter Bot</sub>

---
## linshenkx/prompt-optimizer

> [!info]
> ⭐ 23,513 · TypeScript · 2026-03-02  
> [GitHub](https://github.com/linshenkx/prompt-optimizer) · [Website](https://prompt.always200.com)  
> `#提示词优化` `#多模型集成` `#AI提示工程` `#图像生成` `#llm` `#prompt` `#prompt-engineering` `#prompt-optimization` `#prompt-toolkit` `#prompt-tuning` 
> Prompt Optimizer 是一款专注于提升AI提示词质量的智能优化工具，支持多模型集成（OpenAI、Gemini、DeepSeek等）、双模式优化、对比测试及图像生成功能。提供Web、桌面、Chrome插件和Docker四种部署方式，采用纯客户端架构保障数据安全，适用于角色扮演、知识图谱提取、创意写作等场景，助力开发者降本增效。
> <sub>一款提示词优化器，助力于编写高质量的提示词</sub>

---
## wassupjay/n8n-free-templates

> [!info]
> ⭐ 5,546 · N/A · 2025-08-01  
> [GitHub](https://github.com/wassupjay/n8n-free-templates)  
> `#automation` `#automation-templates` `#integration` `#n8n` `#n8n-autom` `#n8n-template` `#no-code-ai` `#no-code-automation` 
> 生成失败
> <sub>A curated set of 200+ plug-and-play n8n workflows that fuse classic automation with today’s AI stack—vector DBs, embeddings, and LLMs. Import any JSON, add your creds, hit Activate, and you’re live. Built to demo, prototype, or drop straight into production.</sub>

---
## bepass-org/oblivion-desktop

> [!info]
> ⭐ 8,225 · TypeScript · 2026-01-01  
> [GitHub](https://github.com/bepass-org/oblivion-desktop) · [Website](https://github.com/bepass-org/oblivion-desktop/releases)  
> `#anticensorship` `#cloudflare` `#desktop` `#endpoint` `#gool` `#linux` `#mac` `#masque` `#oblivion` `#proxy` `#psiphon` `#sing-box` `#vpn` `#warp` `#warp-plus` `#warpinwarp` `#windows` `#wireguard` `#wireguard-ui` `#zerotrust` 
> 生成失败
> <sub>Oblivion Desktop - Unofficial Warp Client for Windows/Mac/Linux</sub>

---
## czlonkowski/n8n-mcp

> [!info]
> ⭐ 14,595 · TypeScript · 2026-03-08  
> [GitHub](https://github.com/czlonkowski/n8n-mcp) · [Website](https://www.n8n-mcp.com/)  
> `#mcp` `#mcp-server` `#n8n` `#workflows` 
> 生成失败
> <sub>A MCP for Claude Desktop / Claude Code / Windsurf / Cursor to build n8n workflows for you</sub>

---
## justlovemaki/AIClient-2-API

> [!info]
> ⭐ 5,081 · JavaScript · 2026-03-08  
> [GitHub](https://github.com/justlovemaki/AIClient-2-API) · [Website](https://aiproxy.justlikemaki.vip)  
> `#API 代理` `#大模型兼容` `#OpenAI 接口` `#模块化架构` `#aicoding` `#free` 
> AIClient-2-API 是一个强大的 API 代理工具，将仅限客户端使用的免费大模型（如 Gemini、Antigravity、Qwen Code、Kiro）转换为兼容 OpenAI 标准的接口，支持每日数千次请求，内置 Claude 模型免费调用，并通过模块化架构实现高可用性与智能故障转移，极大提升 AI 开发效率。
> <sub>Simulates Gemini CLI, Antigravity, Qwen Code, and Kiro client requests, compatible with the OpenAI API. It supports thousands of Gemini model requests per day and offers free use of the built-in Claude model in Kiro. Easily connect to any client via the API, making AI development more efficient!</sub>

---
## xiyewuqiu/new-lmage

> [!info]
> ⭐ 419 · JavaScript · 2026-03-05  
> [GitHub](https://github.com/xiyewuqiu/new-lmage)  
> 
> 生成失败

---
## cfgranda/ps4ds

> [!info]
> ⭐ 570 · Jupyter Notebook · 2026-02-03  
> [GitHub](https://github.com/cfgranda/ps4ds) · [Website](https://www.ps4ds.net/)  
> `#course-materials` `#data-analysis` `#data-science` `#datascience` `#education` `#jupyter-notebook` `#machine-learning` `#open-source` `#probability` `#probability-statistics` `#python` `#statistics` `#statistics-course` `#statistics-learning` `#teaching` `#teaching-materials` `#textbook` 
> 生成失败
> <sub>Probability and Statistics for Data Science: A self-contained introduction to probability and statistics for data science, including a free pdf, 103 Python notebooks using 23 real-world datasets, 118 videos with slides, and solutions to 200 exercises</sub>

---
## EcoPasteHub/EcoPaste

> [!info]
> ⭐ 6,840 · TypeScript · 2025-11-06  
> [GitHub](https://github.com/EcoPasteHub/EcoPaste) · [Website](https://ecopaste.cn)  
> `#application` `#clipboard` `#clipboard-manager` `#cross-platform` `#desktop-app` `#desktop-application` `#linux` `#macos` `#rust` `#tauri` `#tauri-app` `#windows` 
> 生成失败
> <sub>🎉跨平台的剪贴板管理工具 | Cross-platform clipboard management tool</sub>

---
## cf-pages/Telegraph-Image

> [!info]
> ⭐ 4,449 · HTML · 2026-02-28  
> [GitHub](https://github.com/cf-pages/Telegraph-Image) · [Website](https://im.gurl.eu.org)  
> `#cloudflare` `#cloudflare-pages` `#flickr` `#image` `#image-host` `#image-hosting` `#image-sharing` `#imgur` `#serverless` `#telegraph` `#upload-images` 
> 生成失败
> <sub>Image Hosting solution, Flickr/imgur alternative, make it easy for users to share their images. Using Cloudflare Pages and Telegraph.</sub>

---
## nicejade/wealth-tracker

> [!info]
> ⭐ 809 · Svelte · 2026-02-14  
> [GitHub](https://github.com/nicejade/wealth-tracker) · [Website](https://fund.lovejade.cn)  
> `#analyzer` `#axios` `#dayjs` `#fastify` `#flowbite` `#nodejs` `#sequelize` `#sqlite3` `#svelte` `#tailwindcss` `#vite` 
> 生成失败
> <sub>💰生财有迹（Wealth Tracker）是一款专注于个人资产分析的应用程序。其核心功能是：全面记录并展示用户的资产状况，帮助用户轻松了解财务现状；运用 AI 能力，结合每种资产的特性和当前环境，提供适宜的财务建议。</sub>

---
## googleapis/genai-toolbox

> [!info]
> ⭐ 13,328 · Go · 2026-03-09  
> [GitHub](https://github.com/googleapis/genai-toolbox) · [Website](https://googleapis.github.io/genai-toolbox/getting-started/introduction/)  
> `#agent` `#agents` `#ai` `#bigquery` `#clickhouse` `#cockroachdb` `#database` `#elasticsearch` `#firestore` `#genai` `#llm` `#mcp` `#mongodb` `#mysql` `#oracle` `#postgresql` `#redis` `#server` `#spanner` `#tidb` 
> 生成失败
> <sub>MCP Toolbox for Databases is an open source MCP server for databases.</sub>

---
## Shubhamsaboo/awesome-llm-apps

> [!info]
> ⭐ 100,944 · Python · 2026-03-07  
> [GitHub](https://github.com/Shubhamsaboo/awesome-llm-apps) · [Website](https://www.theunwindai.com)  
> `#agents` `#llms` `#python` `#rag` 
> 生成失败
> <sub>Collection of awesome LLM apps with AI Agents and RAG using OpenAI, Anthropic, Gemini and opensource models.</sub>

---
## sansan0/TrendRadar

> [!info]
> ⭐ 48,269 · Python · 2026-03-01  
> [GitHub](https://github.com/sansan0/TrendRadar) · [Website](https://sansan0.github.io/TrendRadar/)  
> `#AI舆情监控` `#多平台聚合` `#智能推送` `#MCP集成` `#ai` `#bark` `#data-analysis` `#docker` `#hot-news` `#llm` `#mail` `#mcp` `#mcp-server` `#news` `#ntfy` `#python` `#rss` `#trending-topics` `#wechat` `#wework` 
> TrendRadar 是一款 AI 驱动的舆情监控与热点筛选工具，支持多平台热点聚合与 RSS 订阅，具备关键词筛选、AI 翻译、智能分析简报推送等功能。集成 MCP 架构实现自然语言对话分析与趋势预测，支持 Docker 部署及微信/飞书/钉钉/Telegram 等多渠道推送，助力用户高效应对信息过载。
> <sub>⭐AI-driven public opinion & trend monitor with multi-platform aggregation, RSS, and smart alerts.🎯 告别信息过载，你的 AI 舆情监控助手与热点筛选工具！聚合多平台热点 +  RSS 订阅，支持关键词精准筛选。AI 翻译 +  AI 分析简报直推手机，也支持接入 MCP 架构，赋能 AI 自然语言对话分析、情感洞察与趋势预测等。支持 Docker ，数据本地/云端自持。集成微信/飞书/钉钉/Telegram/邮件/ntfy/bark/slack 等渠道智能推送。</sub>

---
## samanhappy/mcphub

> [!info]
> ⭐ 1,853 · TypeScript · 2026-03-08  
> [GitHub](https://github.com/samanhappy/mcphub) · [Website](https://mcphubx.com)  
> `#mcp` `#mcp-gateway` `#mcp-hub` `#mcp-router` `#mcp-server` 
> 生成失败
> <sub>A unified hub for centrally managing and dynamically orchestrating multiple MCP servers/APIs into separate endpoints with flexible routing strategies</sub>

---
## jbiaojerry/ebook-treasure-chest

> [!info]
> ⭐ 8,266 · Python · 2026-01-13  
> [GitHub](https://github.com/jbiaojerry/ebook-treasure-chest)  
> 
> 生成失败
> <sub>欢迎来到电子书下载宝库，一个汇聚了各类电子书下载链接的地方。无论你是喜欢阅读经典文学、经管励志、终身学习、职场创业、技术手册还是其他类型的书籍，这里都能满足你的需求。 该库涵盖了帆书app(原樊登读书)、微信读书、京东读书、喜马拉雅等读书app的大部分电子书。</sub>

---
## other-blowsnow/n8n-nodes-wechat-offiaccount

> [!info]
> ⭐ 865 · TypeScript · 2025-10-17  
> [GitHub](https://github.com/other-blowsnow/n8n-nodes-wechat-offiaccount)  
> 
> 生成失败
> <sub>n8n 微信公众号</sub>

---
## punkpeye/awesome-mcp-servers

> [!info]
> ⭐ 82,538 · N/A · 2026-03-08  
> [GitHub](https://github.com/punkpeye/awesome-mcp-servers) · [Website](https://glama.ai/mcp/servers)  
> `#MCP协议` `#AI上下文扩展` `#服务器实现` `#ai` `#mcp` 
> 这是一个精选的 Model Context Protocol (MCP) 服务器集合，涵盖文件访问、数据库连接、API 集成等功能，支持本地与云端服务，提供多语言实现和跨平台支持，助力 AI 模型扩展上下文能力。
> <sub>A collection of MCP servers.</sub>

---
## xxnuo/dns-benchmark

> [!info]
> ⭐ 686 · Go · 2025-10-10  
> [GitHub](https://github.com/xxnuo/dns-benchmark) · [Website](https://bench.dash.2020818.xyz/)  
> 
> 生成失败
> <sub>dnspy 是一个批量 DNS 服务器基准测试工具，用于本地测量全世界的 DNS 服务器的可访问性和性能。生成可视化图表。dnspy is a bulk DNS server benchmarking tool used to measure the local accessibility and performance of DNS servers worldwide. It generates visual charts.</sub>

---
## u14app/deep-research

> [!info]
> ⭐ 4,501 · JavaScript · 2026-02-10  
> [GitHub](https://github.com/u14app/deep-research) · [Website](https://research.u14.app)  
> `#大语言模型` `#深度研究` `#本地隐私` `#知识图谱` `#anthropic` `#deep-research` `#deep-research-api` `#deepresearch` `#deepseek` `#gemini` `#grok` `#mcp-server` `#ollama` `#openai` 
> Deep Research 是一款基于多模态大语言模型（LLM）的深度研究工具，支持 Gemini、OpenAI、Ollama 等主流模型，集成 SSE API 与 MCP 服务器。具备网页搜索、本地知识库、知识图谱生成、研究历史记录等功能，可在约2分钟内生成高质量研究报告，数据全程本地处理，保障隐私安全，支持 Vercel 与 Cloudflare 快速部署。
> <sub>Use any LLMs (Large Language Models) for Deep Research. Support SSE API and MCP server.</sub>

---
## joeseesun/mcp-prompt-server

> [!info]
> ⭐ 646 · JavaScript · 2025-05-20  
> [GitHub](https://github.com/joeseesun/mcp-prompt-server)  
> 
> 生成失败
> <sub>这是一个基于Model Context Protocol (MCP)的服务器，用于根据用户任务需求提供预设的prompt模板，帮助Cline/Cursor/Windsurf...更高效地执行各种任务。服务器将预设的prompt作为工具(tools)返回，以便在Cursor和Windsurf等编辑器中更好地使用。</sub>

---
## stackia/best-windows-apps

> [!info]
> ⭐ 3,734 · N/A · 2025-03-07  
> [GitHub](https://github.com/stackia/best-windows-apps)  
> 
> 生成失败
> <sub>推荐好用、优秀的 Windows 应用</sub>

---
## kanshurichard/enableAppleAI

> [!info]
> ⭐ 3,478 · Shell · 2025-12-14  
> [GitHub](https://github.com/kanshurichard/enableAppleAI)  
> `#Apple Intelligence` `#macOS破解` `#SIP绕过` `#区域限制解除` 
> 该工具通过修改系统配置文件和注入进程，绕过Apple对中国大陆销售Mac设备的Apple Intelligence功能限制，支持macOS 15.4.1及以上版本。无需长期禁用SIP或运行后台服务，可永久启用Siri整合ChatGPT、Apple News等AI功能，并提供地区伪装为美国的选项以解锁更多服务。
> <sub>Enable Apple Intelligence on Macs sold in Mainland China with SIP enabled, tested on MacOS 15.4.1+ and 26+</sub>

---
## uxiaohan/ZYCS-IMG

> [!info]
> ⭐ 310 · Less · 2025-04-14  
> [GitHub](https://github.com/uxiaohan/ZYCS-IMG) · [Website](https://wp-cdn.4ce.cn)  
> 
> 生成失败
> <sub>骤雨重山图床</sub>

---
## certimate-go/certimate

> [!info]
> ⭐ 8,215 · Go · 2026-03-04  
> [GitHub](https://github.com/certimate-go/certimate) · [Website](https://docs.certimate.me)  
> `#acme` `#acme-client` `#automation` `#certbot` `#certificate` `#certificate-management` `#certificate-manager` `#certificates` `#devops` `#https` `#https-certificate` `#lego` `#letsencrypt` `#self-hosted` `#ssl` `#ssl-cert` `#ssl-certificate` `#ssl-certificates` 
> 生成失败
> <sub>An open-source and free self-hosted SSL certificates ACME tool, automates the full-cycle of issuance, deployment, renewal, and monitoring visually. 完全开源免费的自托管 SSL 证书 ACME 工具，申请、部署、续期、监控全流程自动化可视化，支持各大主流云厂商。</sub>

---
## cmliu/SubsCheck-Win-GUI

> [!info]
> ⭐ 4,331 · C# · 2025-04-24  
> [GitHub](https://github.com/cmliu/SubsCheck-Win-GUI) · [Website](https://youtu.be/sS9Tuf1PCyc)  
> `#proxypool` 
> 生成失败
> <sub>SubsCheck 为 Windows 用户设计的 GUI 程序界面。</sub>

---
## risin42/NagramX

> [!info]
> ⭐ 1,404 · Java · 2026-03-09  
> [GitHub](https://github.com/risin42/NagramX)  
> 
> 生成失败
> <sub>A variant of Nagram with additional features.</sub>

---
## bestruirui/BestSub

> [!info]
> ⭐ 2,022 · TypeScript · 2026-03-05  
> [GitHub](https://github.com/bestruirui/BestSub) · [Website](https://t.me/TheBestSubGroup)  
> `#clash` `#mihomo` `#sing-box` 
> 生成失败
> <sub>Best Sub, Best for Your Net</sub>

---
## liyown/ai-trend-publish

> [!info]
> ⭐ 2,807 · TypeScript · 2026-03-07  
> [GitHub](https://github.com/liyown/ai-trend-publish) · [Website](https://liyown.github.io/ai-trend-publish/)  
> `#ai` `#weixin` 
> 生成失败
> <sub>TrendPublish: 全自动 AI 内容生成与发布系统 | 微信公众号自动化 | 多源数据抓取 (Twitter/X、网站) | DeepseekAI、千问、讯飞模型 | 智能内容分析排序 | 定时发布 | 多模板支持 | Node.js | TypeScript | AI 技术趋势跟踪工具</sub>

---
## knowledgefxg/learning-english

> [!info]
> ⭐ 3,146 · N/A · 2026-02-24  
> [GitHub](https://github.com/knowledgefxg/learning-english) · [Website](https://en.knowledgefxg.com/)  
> 
> 生成失败
> <sub>精选优质英语学习资源合集，专注于听说读写等核心技能的提升。包含语法、词汇和媒体资源，助您更好地学习英语。</sub>

---
## Anxcye/anx-reader

> [!info]
> ⭐ 7,833 · Dart · 2026-03-07  
> [GitHub](https://github.com/Anxcye/anx-reader) · [Website](https://anx.anxcye.com)  
> `#电子书阅读器` `#AI 辅助阅读` `#多平台同步` `#笔记管理` `#dart` `#ebook-reader` `#flutter` 
> Anx Reader 是一款专为阅读爱好者打造的多平台电子书阅读器，支持 EPUB、MOBI、PDF 等主流格式，集成 AI 助手实现智能书架整理、思维导图生成、词典翻译与内容摘要。提供深度个性化阅读设置、笔记管理、阅读数据统计及 TTS 朗读等功能，致力于提升专注力与阅读体验。
> <sub>Featuring powerful AI capabilities and supporting various e-book formats, it makes reading smarter and more focused.</sub>

---
## OpenHealthForAll/open-health

> [!info]
> ⭐ 3,828 · TypeScript · 2026-01-06  
> [GitHub](https://github.com/OpenHealthForAll/open-health) · [Website](https://www.open-health.me)  
> `#ai` `#healthcare` 
> 生成失败
> <sub>OpenHealth, AI Health Assistant | Powered by Your Data</sub>

---
## houjoe0829/shadowrocket-rules-joe

> [!info]
> ⭐ 66 · Python · 2026-02-26  
> [GitHub](https://github.com/houjoe0829/shadowrocket-rules-joe)  
> 
> 生成失败
> <sub>放置我的 Shadowrocket 规则，在 GitHub 里管理和更新</sub>

---
## MarSeventh/CloudFlare-ImgBed

> [!info]
> ⭐ 4,538 · JavaScript · 2026-03-08  
> [GitHub](https://github.com/MarSeventh/CloudFlare-ImgBed) · [Website](https://cfbed.sanyue.de)  
> `#cloudflare-pages` `#cloudflare-r2` `#discord` `#gallery` `#huggingface` `#imgbed` `#netdisk` `#s3` `#telegram-bot` 
> 生成失败
> <sub>Open-source file hosting solution based on CloudFlare  (Image hosting/File storage/Cloud drive) / 基于 CloudFlare 的开源文件托管解决方案（图床/文件床/网盘）</sub>

---
## deepseek-ai/awesome-deepseek-integration

> [!info]
> ⭐ 35,821 · N/A · 2026-02-23  
> [GitHub](https://github.com/deepseek-ai/awesome-deepseek-integration)  
> `#awesome` `#deepseek` 
> 生成失败
> <sub>Integrate the DeepSeek API into popular software</sub>

---
## foamzou/melody

> [!info]
> ⭐ 3,715 · JavaScript · 2026-02-19  
> [GitHub](https://github.com/foamzou/melody)  
> `#music` 
> 生成失败
> <sub>我的音乐精灵</sub>

---
## PDFMathTranslate/PDFMathTranslate

> [!info]
> ⭐ 32,114 · Python · 2025-11-25  
> [GitHub](https://github.com/PDFMathTranslate/PDFMathTranslate) · [Website](https://pdf2zh.com)  
> `#chinese` `#document` `#edit` `#english` `#japanese` `#korean` `#latex` `#math` `#mcp` `#modify` `#obsidian` `#openai` `#pdf` `#pdf2zh` `#python` `#russian` `#translate` `#translation` `#zotero` 
> 生成失败
> <sub>[EMNLP 2025 Demo] PDF scientific paper translation with preserved formats - 基于 AI 完整保留排版的 PDF 文档全文双语翻译，支持 Google/DeepL/Ollama/OpenAI 等服务，提供 CLI/GUI/MCP/Docker/Zotero</sub>

---
## SteveSuv/remix-words-funny

> [!info]
> ⭐ 921 · TypeScript · 2026-01-31  
> [GitHub](https://github.com/SteveSuv/remix-words-funny) · [Website](https://wordsfunny.com)  
> `#docker` `#drizzle-orm` `#heroui` `#jotai` `#jwt` `#lucide-icons` `#nodemailer` `#postgresql` `#react` `#react-query` `#react-router` `#remix` `#tailwindcss` `#trpc` `#typescript` `#vite` 
> 生成失败
> <sub>a fullstack English words study website: remix (react-router v7) + trpc + drizzle + postgresql + heroui + jotai + docker</sub>

---
## 7Sageer/sublink-worker

> [!info]
> ⭐ 4,294 · JavaScript · 2026-03-09  
> [GitHub](https://github.com/7Sageer/sublink-worker) · [Website](https://sublink.works/)  
> `#clash` `#clash-meta` `#cloudflare-workers` `#mihomo` `#singbox` `#v2rayn` `#v2rayng` `#xray-core` 
> 生成失败
> <sub>One Worker, All Subscriptions</sub>

---
## lijigang/write-prompt

> [!info]
> ⭐ 571 · N/A · 2024-09-28  
> [GitHub](https://github.com/lijigang/write-prompt) · [Website](http://www.lijigang.com)  
> 
> 生成失败
> <sub>公众号Write Prompt 发布的Prompt，同步记录于此</sub>

---
## amxamxa/paywall-killer

> [!info]
> ⭐ 51 · N/A · 2025-02-09  
> [GitHub](https://github.com/amxamxa/paywall-killer)  
> 
> 生成失败
> <sub>KI-prompt for unlocking 92,25% of all paywalls</sub>

---
## kingkare/GLaDOS

> [!info]
> ⭐ 22 · Python · 2024-11-27  
> [GitHub](https://github.com/kingkare/GLaDOS)  
> 
> 生成失败
> <sub>GLaDOS自动签到</sub>

---
## trungdq88/Awesome-Black-Friday-Cyber-Monday

> [!info]
> ⭐ 7,479 · N/A · 2025-11-30  
> [GitHub](https://github.com/trungdq88/Awesome-Black-Friday-Cyber-Monday)  
> 
> 生成失败
> <sub>Awesome apps, software, and SaaS deals on Black Friday.</sub>

---
## liujuntao123/chines-history-video

> [!info]
> ⭐ 820 · Vue · 2024-10-31  
> [GitHub](https://github.com/liujuntao123/chines-history-video) · [Website](https://historyline.19921014.xyz)  
> 
> 生成失败
> <sub>中国历朝代历史视频讲解</sub>

---
## Reno9527/awesome-xiaobot

> [!info]
> ⭐ 232 · N/A · 2025-03-08  
> [GitHub](https://github.com/Reno9527/awesome-xiaobot)  
> 
> 生成失败
> <sub>小报童大全 -  小报童排行榜</sub>

---
## berryjam/mastering-bitcoin-3rd

> [!info]
> ⭐ 248 · N/A · 2025-10-11  
> [GitHub](https://github.com/berryjam/mastering-bitcoin-3rd) · [Website](https://berryjam.gitbook.io/mastering-bitcoin-3rd)  
> 
> 生成失败
> <sub>《精通比特币》第三版翻译，希望本翻译内容能够为大家带来更新的比特币学习内容和一些技术上的思考，内含比特币java应用开发示例、rust的brc20编程实践，使用cpuminer挖矿，BTC二层详细技术分析，欢迎pr提交与BTC相关的技术知识或者修正优化文中错误不足。</sub>

---
## wechat-article/wechat-article-exporter

> [!info]
> ⭐ 7,342 · TypeScript · 2026-03-05  
> [GitHub](https://github.com/wechat-article/wechat-article-exporter) · [Website](https://down.mptext.top)  
> `#download` `#wechat` `#wechat-article` `#wechat-download` 
> 生成失败
> <sub>一款在线的 微信公众号文章批量下载 工具，支持导出阅读量与评论数据，无需搭建任何环境，可通过 在线网站 使用，支持 docker 私有化部署和 Cloudflare 部署。  支持下载各种文件格式，其中 HTML 格式可100%还原文章排版与样式。</sub>

---
## interaminense/learning-english

> [!info]
> ⭐ 661 · N/A · 2026-02-19  
> [GitHub](https://github.com/interaminense/learning-english)  
> `#duolingo` `#english-grammar` `#english-language` `#hacktoberfest-accepted` `#learn-english` `#learning-english` `#lingualeo` 
> 生成失败
> <sub>:gb: Useful links that will help me (and probably you) to learn English through the Internet.</sub>

---
## NeoVertex1/SuperPrompt

> [!info]
> ⭐ 6,382 · N/A · 2025-09-03  
> [GitHub](https://github.com/NeoVertex1/SuperPrompt)  
> `#ai` `#ml` `#prompt-engineering` `#prompts` `#prompts-template` 
> 生成失败
> <sub>SuperPrompt is an attempt to engineer prompts that might help us understand AI agents.</sub>

---
## kenjihiranabe/The-Art-of-Linear-Algebra

> [!info]
> ⭐ 21,100 · PostScript · 2025-06-30  
> [GitHub](https://github.com/kenjihiranabe/The-Art-of-Linear-Algebra)  
> 
> 生成失败
> <sub>Graphic notes on Gilbert Strang's "Linear Algebra for Everyone"</sub>

---
## bookstairs/bookhunter

> [!info]
> ⭐ 1,245 · Go · 2026-02-28  
> [GitHub](https://github.com/bookstairs/bookhunter)  
> `#epub` `#golang` `#spider` 
> 生成失败
> <sub>A download tools for clawing the ebooks from internets.</sub>

---
## guobao2333/DeepLX-Serverless

> [!info]
> ⭐ 553 · JavaScript · 2025-08-15  
> [GitHub](https://github.com/guobao2333/DeepLX-Serverless)  
> `#api` `#deepl` `#deeplx` `#free-api` `#i18n` `#javascript` `#nodejs` `#serverless` `#translation` 
> 生成失败
> <sub>Free DeepL API for Serverless</sub>

---
## lanqian528/chat2api

> [!info]
> ⭐ 3,391 · Python · 2025-05-17  
> [GitHub](https://github.com/lanqian528/chat2api)  
> 
> 生成失败
> <sub>A service that can convert ChatGPT on the web to OpenAI API format.</sub>

---
## hiddify/hiddify-app

> [!info]
> ⭐ 27,059 · Dart · 2026-03-05  
> [GitHub](https://github.com/hiddify/hiddify-app) · [Website](https://hiddify.com)  
> `#代理客户端` `#Sing-box` `#多协议支持` `#跨平台` `#clash` `#clashmeta` `#ech` `#hysteria` `#hysteria2` `#proxy` `#reality` `#shadowsocks` `#shadowtls` `#sing-box` `#singbox` `#ssh` `#tuic` `#v2ray` `#vless` `#vmess` `#wireguard` `#xray` 
> Hiddify 是一款基于 Sing-box 的多平台代理客户端，支持 Vless、Vmess、Reality、TUIC、Hysteria、Wireguard、SSH 等协议，具备自动节点选择、TUN 模式、远程配置订阅等功能。开源无广告，适用于 Android、iOS、Windows、macOS 和 Linux，专为伊朗、中国、俄罗斯等地区优化，提供安全私密的网络访问。
> <sub>Multi-platform auto-proxy client, supporting Sing-box, X-ray, TUIC, Hysteria, Reality, Trojan, SSH etc. It’s an open-source, secure and ad-free.</sub>

---
## KotatsuApp/Kotatsu

> [!info]
> ⭐ 8,343 · Kotlin · 2025-11-04  
> [GitHub](https://github.com/KotatsuApp/Kotatsu) · [Website](https://kotatsu.app)  
> `#android` `#comics` `#manga` `#manga-reader` `#mangareader` `#webtoon` 
> 生成失败
> <sub>Manga reader for Android</sub>

---
## fatwang2/search2ai

> [!info]
> ⭐ 1,285 · JavaScript · 2025-02-19  
> [GitHub](https://github.com/fatwang2/search2ai) · [Website](https://sum4all.site/telegram)  
> `#functioncalling` `#gemini` `#groq` `#llama` `#mistral` `#online` `#openai` `#openai-api` `#search` `#toolcall` 
> 生成失败
> <sub>Help your LLMs online</sub>

---
## HqWu-HITCS/Awesome-Chinese-LLM

> [!info]
> ⭐ 22,338 · N/A · 2025-05-19  
> [GitHub](https://github.com/HqWu-HITCS/Awesome-Chinese-LLM)  
> `#awesome-lists` `#chatglm` `#chinese` `#llama` `#llm` `#nlp` 
> 生成失败
> <sub>整理开源的中文大语言模型，以规模较小、可私有化部署、训练成本较低的模型为主，包括底座模型，垂直领域微调及应用，数据集与教程等。</sub>

---
## ChatGPTNextWeb/NextChat

> [!info]
> ⭐ 87,445 · TypeScript · 2025-12-02  
> [GitHub](https://github.com/ChatGPTNextWeb/NextChat) · [Website](https://nextchat.club)  
> `#多模型AI助手` `#跨平台应用` `#企业私有化部署` `#calclaude` `#chatgpt` `#claude` `#cross-platform` `#desktop` `#fe` `#gemini` `#gemini-pro` `#gemini-server` `#gemini-ultra` `#gpt-4o` `#groq` `#nextjs` `#ollama` `#react` `#tauri` `#tauri-app` `#vercel` `#webui` 
> NextChat 是一款轻量快速的 AI 助手，支持 Claude、DeepSeek、GPT-4 和 Gemini Pro 等多模型。提供跨平台支持（Web、iOS、macOS、Android、Linux、Windows），具备企业版私有化部署能力，支持品牌定制、权限控制、知识库集成与安全审计，适用于个人与企业场景。
> <sub>✨ Light and Fast AI Assistant. Support: Web | iOS | MacOS | Android |  Linux | Windows</sub>

---
## cmliu/WorkerVless2sub

> [!info]
> ⭐ 5,613 · JavaScript · 2026-03-08  
> [GitHub](https://github.com/cmliu/WorkerVless2sub) · [Website](https://VLESS.fxxk.dedyn.io)  
> 
> 生成失败
> <sub>自动化批量替换生成优选线路 VMess / VLESS / Trojan 节点的 优选订阅生成器</sub>

---
## zhuima/awesome-cloudflare

> [!info]
> ⭐ 12,986 · N/A · 2026-02-28  
> [GitHub](https://github.com/zhuima/awesome-cloudflare) · [Website](https://cloudflare.chuhai.tools/)  
> 
> 生成失败
> <sub>⛅️ 精选的 Cloudflare 工具、开源项目、指南、博客和其他资源列表。/ ⛅️ A curated list of Cloudflare tools, open source projects, guides, blogs and other resources.</sub>

---
## malinkang/weread2notion-pro

> [!info]
> ⭐ 3,412 · Python · 2025-07-24  
> [GitHub](https://github.com/malinkang/weread2notion-pro)  
> 
> 生成失败

---
## XIU2/CloudflareSpeedTest

> [!info]
> ⭐ 24,916 · Go · 2026-03-02  
> [GitHub](https://github.com/XIU2/CloudflareSpeedTest)  
> `#cdn` `#cloudflare` `#go` `#golang` `#speedtest` 
> 生成失败
> <sub>🌩「自选优选 IP」测试 Cloudflare CDN 延迟和速度，获取最快 IP ！当然也支持其他 CDN / 多个解析 IP 的网站 ~</sub>

---
## AttemptD/AfuseKt-release

> [!info]
> ⭐ 4,142 · N/A · 2026-02-27  
> [GitHub](https://github.com/AttemptD/AfuseKt-release)  
> 
> 生成失败

---
## vvbbnn00/WARP-Clash-API

> [!info]
> ⭐ 8,856 · Python · 2024-09-04  
> [GitHub](https://github.com/vvbbnn00/WARP-Clash-API)  
> `#clash` `#cloudflare` `#docker` `#docker-compose` `#flask` `#shadowrocket` `#warp` `#warp-plus` 
> 生成失败
> <sub>该项目可以让你通过订阅的方式使用Cloudflare WARP+，自动获取流量。This project enables you to use Cloudflare WARP+ through subscription, automatically acquiring traffic.</sub>

---
## CrazyCreativeDream/Real-Coze-API

> [!info]
> ⭐ 129 · JavaScript · 2024-04-12  
> [GitHub](https://github.com/CrazyCreativeDream/Real-Coze-API)  
> 
> 生成失败
> <sub>高性能的真·Coze API</sub>

---
## Kuingsmile/PicList

> [!info]
> ⭐ 3,401 · Vue · 2026-03-07  
> [GitHub](https://github.com/Kuingsmile/PicList) · [Website](https://piclist.cn)  
> `#alioss` `#electron` `#element-plus` `#github` `#imgur` `#imgur-uploader` `#picgo` `#qiniu` `#s3` `#smms` `#tencent-cos` `#upyun` `#vue` `#webdav` `#webdav-client` 
> 生成失败
> <sub>An image upload and manage tool, base on PicGo</sub>

---
## xlucn/oh-my-foss-android

> [!info]
> ⭐ 4,861 · N/A · 2025-09-05  
> [GitHub](https://github.com/xlucn/oh-my-foss-android)  
> `#android` `#apps` `#awesome` `#f-droid` `#foss` `#open-source` `#opensource` 
> 生成失败
> <sub>个人收集的实用、良心开源安卓软件</sub>

---
## NodeBE4/impact

> [!info]
> ⭐ 160 · HTML · 2024-07-18  
> [GitHub](https://github.com/NodeBE4/impact) · [Website](https://nodebe4.github.io/impact/)  
> 
> 生成失败
> <sub>Github social impact projects</sub>

---
## jeffyus/renwenjiangzuo

> [!info]
> ⭐ 52 · N/A · 2023-01-29  
> [GitHub](https://github.com/jeffyus/renwenjiangzuo)  
> 
> 生成失败
> <sub>抢救中文人文讲座，建立人文讲座列表</sub>

---
## qianguyihao/blog-list

> [!info]
> ⭐ 2,678 · N/A · 2026-03-04  
> [GitHub](https://github.com/qianguyihao/blog-list)  
> 
> 生成失败
> <sub>中文博客琅琊榜，只收录精品独立博客</sub>

---
## linexjlin/GPTs

> [!info]
> ⭐ 31,978 · N/A · 2024-09-27  
> [GitHub](https://github.com/linexjlin/GPTs)  
> `#提示工程` `#GPT定制` `#泄露提示词` 
> 该仓库收集了多个泄露的 GPTs 提示词，涵盖开发、翻译、编程辅助、内容生成、角色扮演等多个应用场景，来源包括社交媒体、用户提交及逆向分析，为研究提示工程与 GPT 定制化提供了丰富样本。
> <sub>leaked prompts of GPTs</sub>

---
## msterzhang/onelist

> [!info]
> ⭐ 1,536 · Go · 2024-01-12  
> [GitHub](https://github.com/msterzhang/onelist)  
> 
> 生成失败
> <sub>一个类似emby的专注于刮削alist聚合网盘形成影视媒体库的程序。</sub>

---
## LetTTGACO/elog

> [!info]
> ⭐ 1,834 · TypeScript · 2025-03-06  
> [GitHub](https://github.com/LetTTGACO/elog) · [Website](https://elog.1874.cool)  
> `#blog` `#confluence` `#docusaurus` `#docz` `#feishu` `#flowus` `#hexo` `#hugo` `#markdown` `#notion` `#vitepress` `#wordpress` `#yuque` 
> 生成失败
> <sub>Markdown 批量导出工具、开放式跨平台博客解决方案，随意组合写作平台(语雀/Notion/FlowUs/飞书/我来Wolai)和博客平台(Hexo/Vitepress/Halo/Confluence/WordPress等)</sub>

---
## lobehub/lobehub

> [!info]
> ⭐ 73,296 · TypeScript · 2026-03-09  
> [GitHub](https://github.com/lobehub/lobehub) · [Website](https://lobehub.com)  
> `#agent` `#agent-collaboration` `#agent-harness` `#ai` `#chatgpt` `#claude` `#deepseek` `#gemini` `#gpt` `#knowledge-base` `#mcp` `#openai` 
> 生成失败
> <sub>The ultimate space for work and life — to find, build, and collaborate with agent teammates that grow with you. We are taking agent harness to the next level — enabling multi-agent collaboration, effortless agent team design, and introducing agents as the unit of work interaction.</sub>

---
## Xatta-Trone/medium-parser-extension

> [!info]
> ⭐ 1,588 · JavaScript · 2025-12-01  
> [GitHub](https://github.com/Xatta-Trone/medium-parser-extension)  
> `#medium` `#medium-article` `#medium-com` 
> 生成失败
> <sub>Read medium.com and medium based articles using google web cache.</sub>

---
## hiroi-sora/Umi-OCR

> [!info]
> ⭐ 42,447 · Python · 2025-11-20  
> [GitHub](https://github.com/hiroi-sora/Umi-OCR)  
> `#OCR识别` `#离线文字识别` `#批量PDF处理` `#二维码生成` `#ocr` `#ocr-python` `#paddleocr` `#qml` `#qt` `#screenshot` `#umi-ocr` 
> Umi-OCR 是一款开源、免费且完全离线的文字识别工具，支持截图识别、批量图片/PDF处理、二维码扫描与生成，内置多语言OCR引擎，可排除水印区域，并提供命令行与HTTP接口调用，适用于Windows和Linux系统。
> <sub>OCR software, free and offline. 开源、免费的离线OCR软件。支持截屏/批量导入图片，PDF文档识别，排除水印/页眉页脚，扫描/生成二维码。内置多国语言库。</sub>

---
## ninxsoft/Mist

> [!info]
> ⭐ 4,690 · Swift · 2025-10-11  
> [GitHub](https://github.com/ninxsoft/Mist)  
> 
> 生成失败
> <sub>A Mac utility that automatically downloads macOS Firmwares / Installers.</sub>

---
## getActivity/AndroidIndex

> [!info]
> ⭐ 1,493 · N/A · 2026-02-04  
> [GitHub](https://github.com/getActivity/AndroidIndex)  
> 
> 生成失败
> <sub>Android 资源大汇总</sub>

---
## microsoft/Data-Science-For-Beginners

> [!info]
> ⭐ 34,163 · Jupyter Notebook · 2026-03-05  
> [GitHub](https://github.com/microsoft/Data-Science-For-Beginners)  
> `#data-analysis` `#data-science` `#data-visualization` `#microsoft-for-beginners` `#pandas` `#python` 
> 生成失败
> <sub>10 Weeks, 20 Lessons, Data Science for All!</sub>

---
## Loyalsoldier/clash-rules

> [!info]
> ⭐ 24,939 · N/A · 2026-03-08  
> [GitHub](https://github.com/Loyalsoldier/clash-rules)  
> `#Clash规则集` `#科学上网` `#域名分流` `#广告拦截` `#adblock` `#adguard` `#anticensorship` `#chinalist` `#clash` `#dnsmasq` `#easylist` `#geosite` `#gfw` `#gfwlist` `#proxy` `#routing` `#shadowsocks` `#ss` `#ssr` `#surge` `#v2ray` 
> 该项目为 Clash Premium 内核提供高质量规则集（RULE-SET），涵盖直连、代理与广告拦截域名列表，兼容 ClashX Pro、Clash for Windows 等主流客户端。数据源自 v2ray-rules-dat 与 dnsmasq-china-list，每日自动更新，支持 TUN 模式全流量接管，适用于多平台科学上网与网络优化。
> <sub>🦄️ 🎃 👻 Clash Premium 规则集(RULE-SET)，兼容 ClashX Pro、Clash for Windows 等基于 Clash Premium 内核的客户端。</sub>

---
## spiritLHLS/ecs

> [!info]
> ⭐ 6,455 · Shell · 2026-02-28  
> [GitHub](https://github.com/spiritLHLS/ecs) · [Website](https://t.me/+UHVoo2U4VyA5NTQ1)  
> `#almalinux` `#arch` `#astralinux` `#bench-script` `#benchmark` `#cdn` `#centos` `#checker` `#debian` `#fedora` `#goecs` `#ipv6` `#lemonbench` `#openai` `#oracle-linux` `#rockylinux` `#speedtest` `#sysbench` `#ubuntu` `#vps` 
> 生成失败
> <sub>VPS 融合怪服务器测评项目 更推荐使用无环境依赖的Go版本 VPS Fusion Monster Server Test Script – More recommended to use the Go version with no environment dependencies: https://github.com/oneclickvirt/ecs</sub>

---
## fanmingming/live

> [!info]
> ⭐ 27,791 · JavaScript · 2026-02-17  
> [GitHub](https://github.com/fanmingming/live) · [Website](https://live.fanmingming.com/)  
> `#china` `#converter` `#epg` `#iptv` `#ipv6` `#live` `#m3u` `#m3u8` `#mp4` `#radio` `#television` `#tv` `#txt` `#workers` 
> 生成失败
> <sub>✯ 可直连访问的电视/广播图标库与相关工具项目 ✯ 🔕 永久免费 直连访问 完整开源 不断完善的台标 支持IPv4/IPv6双栈访问 🔕</sub>

---
## gaozhangmin/aliyunpan

> [!info]
> ⭐ 5,921 · TypeScript · 2025-09-22  
> [GitHub](https://github.com/gaozhangmin/aliyunpan) · [Website](https://github.com/gaozhangmin/aliyunpan)  
> `#electron-app` `#linux` `#macos` `#typescript` `#vue3` `#windows` 
> 生成失败
> <sub>小白羊网盘 - Powered by 阿里云盘。</sub>

---
## Mayandev/where-is-douban250

> [!info]
> ⭐ 706 · Python · 2022-03-20  
> [GitHub](https://github.com/Mayandev/where-is-douban250) · [Website](https://mayandev.top/where-is-douban250/)  
> `#bilibili` `#douban` `#movie` 
> 生成失败
> <sub>🐛 一个爬虫程序，整理了腾讯视频、爱奇艺、优酷、哔哩哔哩等视频网站中，能够观看的「豆瓣电影 Top250 榜单」影片。</sub>

---
## 217heidai/adblockfilters

> [!info]
> ⭐ 6,442 · Python · 2026-03-09  
> [GitHub](https://github.com/217heidai/adblockfilters)  
> `#adblock` `#adguard` `#adguardhome` `#clash` `#dnsmasq` `#fiters` `#hosts` `#invizible` `#loon` `#mihomo` `#personaldnsfilter` `#quantumultx` `#shadowrocket` `#sing-box` `#smartdns` 
> 生成失败
> <sub>去广告合并规则，每8个小时更新一次。</sub>

---
## chatanywhere/GPT_API_free

> [!info]
> ⭐ 36,486 · Python · 2026-03-06  
> [GitHub](https://github.com/chatanywhere/GPT_API_free) · [Website](https://api.chatanywhere.tech)  
> `#大模型API` `#免费中转` `#OpenAI兼容` `#api` `#chatgpt` `#claude` `#deepseek` `#gemini` `#gpt` `#grok` 
> 该项目提供免费且稳定的 GPT、DeepSeek、Claude、Gemini 等主流大模型 API 转发服务，支持国内直连无需代理，兼容 OpenAI 标准协议，接口一致、接入便捷。免费版每日限额调用，付费版支持更高性能与无限使用，适合个人开发、教育及非商用场景。
> <sub>Free ChatGPT&DeepSeek API Key，免费ChatGPT&DeepSeek API。免费接入DeepSeek API和GPT4 API，支持 gpt | deepseek | claude | gemini | grok 等排名靠前的常用大模型。</sub>

---
## getsomecat/GetSomeCats

> [!info]
> ⭐ 2,183 · JavaScript · 2024-03-23  
> [GitHub](https://github.com/getsomecat/GetSomeCats)  
> 
> 生成失败

---
## layou233/NeverIdle

> [!info]
> ⭐ 884 · Go · 2026-03-08  
> [GitHub](https://github.com/layou233/NeverIdle)  
> `#oracle-cloud` `#speedtest` 
> 生成失败
> <sub>资源定期浪费，可用于 Oracle 甲骨文保活。</sub>

---
## gps949/MirageSyno

> [!info]
> ⭐ 16 · Go · 2023-10-17  
> [GitHub](https://github.com/gps949/MirageSyno)  
> 
> 生成失败
> <sub>蜃境群晖客户端</sub>

---
## hehonghui/awesome-english-ebooks

> [!info]
> ⭐ 29,431 · CSS · 2026-03-06  
> [GitHub](https://github.com/hehonghui/awesome-english-ebooks)  
> `#download` `#ebooks` `#economist` `#economist-ebooks` `#new-yorker` `#pdf` 
> 生成失败
> <sub>经济学人(含音频)、纽约客、卫报、连线、大西洋月刊等英语杂志免费下载,支持epub、mobi、pdf格式, 每周更新</sub>

---
## wdxtub/WDXpeak

> [!info]
> ⭐ 883 · Stata · 2021-10-17  
> [GitHub](https://github.com/wdxtub/WDXpeak)  
> 
> 生成失败
> <sub>我的各类笔记（~ 至 2016.12）</sub>

---
## jxxghp/MoviePilot

> [!info]
> ⭐ 10,646 · Python · 2026-03-09  
> [GitHub](https://github.com/jxxghp/MoviePilot) · [Website](https://movie-pilot.org)  
> 
> 生成失败
> <sub>NAS媒体库自动化管理工具</sub>

---
## XPoet/picx

> [!info]
> ⭐ 5,066 · TypeScript · 2025-02-26  
> [GitHub](https://github.com/XPoet/picx) · [Website](https://picx.xpoet.cn)  
> `#cdn` `#cloudflare` `#github-api` `#image-hosting` `#image-toolbox` `#jsdelivr` `#picx` `#staticaly` 
> 生成失败
> <sub>🏞️ PicX 是一款基于 GitHub API 开发的图床工具，提供图片上传托管、生成图片链接和常用图片工具箱服务。</sub>

---
## VPN-Subcription-Links/ClashX-V2Ray-TopFreeProxy

> [!info]
> ⭐ 5,074 · Python · 2024-07-25  
> [GitHub](https://github.com/VPN-Subcription-Links/ClashX-V2Ray-TopFreeProxy)  
> `#clash` `#clash-for-windows` `#clashx` `#clashx-pro` `#free` `#free-node` `#free-proxy` `#free-vpn` `#proxy` `#proxy-pool` `#shadowrocket` `#ss` `#ssr` `#trojan` `#tutorial` `#v2ray` `#vmess` `#vpn` 
> 生成失败
> <sub>Top free VPN (ClashX & V2Ray proxy) with subscription links. [免费VPN、免费梯子、免费科学上网、免费订阅链接、免费节点、精选、ClashX & V2Ray 教程]</sub>

---
## megaease/Remembering-Haoel

> [!info]
> ⭐ 2,944 · N/A · 2024-05-23  
> [GitHub](https://github.com/megaease/Remembering-Haoel)  
> 
> 生成失败
> <sub>记录您对左耳朵耗子（陈皓）的点滴回忆</sub>

---
## OdysseusYuan/LKY_OfficeTools

> [!info]
> ⭐ 11,442 · C# · 2024-02-22  
> [GitHub](https://github.com/OdysseusYuan/LKY_OfficeTools) · [Website](Email: OdysseusYuan@foxmail.com)  
> `#access` `#email` `#excel` `#kms` `#lync` `#microsoft` `#msoffice` `#office` `#office-free` `#office365` `#onedrive` `#onenote` `#outlook` `#ppt` `#project` `#publisher` `#skype` `#teams` `#visio` `#word` 
> 生成失败
> <sub>一键自动化 下载、安装、激活 Office 的利器。</sub>

---
## Johnshall/Shadowrocket-ADBlock-Rules-Forever

> [!info]
> ⭐ 23,401 · N/A · 2026-03-08  
> [GitHub](https://github.com/Johnshall/Shadowrocket-ADBlock-Rules-Forever) · [Website](https://johnshall.github.io/Shadowrocket-ADBlock-Rules-Forever/)  
> `#adblock` `#gfw` `#proxy` `#shadowrocket` `#v2ray` 
> 生成失败
> <sub>提供多款 Shadowrocket 规则，拥有强劲的广告过滤功能。每日 8 时重新构建规则。</sub>

---
## PlexPt/awesome-chatgpt-prompts-zh

> [!info]
> ⭐ 58,601 · N/A · 2025-01-01  
> [GitHub](https://github.com/PlexPt/awesome-chatgpt-prompts-zh) · [Website](https://chat.aimakex.com/)  
> `#ChatGPT提示工程` `#中文AI写作` `#大模型应用指南` `#chat-gpt` `#chatgpt` `#chatgpt3` `#chatgpt4` `#gpt` 
> 本仓库提供全面的ChatGPT中文调教指南，涵盖学术论文、创意写作、商业文案、技术文档、翻译、数据分析等数十种应用场景，帮助用户掌握高效提示词设计技巧，充分发挥GPT-3.5与GPT-4的生成能力，提升AI输出质量与实用性。
> <sub>ChatGPT 中文调教指南。各种场景使用指南。学习怎么让它听你的话。</sub>

---
## sub-store-org/Sub-Store

> [!info]
> ⭐ 8,844 · JavaScript · 2026-03-07  
> [GitHub](https://github.com/sub-store-org/Sub-Store)  
> `#clash` `#http` `#loon` `#quantumultx` `#shadowrocket` `#shadowsocks` `#shadowsocksr` `#ssr` `#stash` `#surge4` `#trojan` `#vmess` 
> 生成失败
> <sub>Advanced Subscription Manager for QX, Loon, Surge, Stash, Egern and Shadowrocket!</sub>

---
## PKUFlyingPig/cs-self-learning

> [!info]
> ⭐ 71,667 · HTML · 2026-02-24  
> [GitHub](https://github.com/PKUFlyingPig/cs-self-learning) · [Website](https://csdiy.wiki)  
> 
> 生成失败
> <sub>计算机自学指南</sub>

---
## imDazui/Tvlist-awesome-m3u-m3u8

> [!info]
> ⭐ 29,000 · N/A · 2025-11-14  
> [GitHub](https://github.com/imDazui/Tvlist-awesome-m3u-m3u8)  
> `#awesome` `#iptv` `#iptv-channels` `#iptv-free` `#iptv-m3u8` `#iptv-playlist` `#kodi` `#kodi-addons` `#kodi-plugin` `#m3u` `#m3u8` 
> 生成失败
> <sub>直播源相关资源汇总 📺 💯 IPTV、M3U —— 勤洗手、戴口罩，祝愿所有人百毒不侵</sub>

---
## superleeyom/my-feed-OPML

> [!info]
> ⭐ 171 · Java · 2023-05-24  
> [GitHub](https://github.com/superleeyom/my-feed-OPML) · [Website](https://github.com/superleeyom/my-feed-OPML/releases/download/latest/feed.opml)  
> `#blog` `#feed` `#feedly` `#newsletter` `#opml` 
> 生成失败
> <sub>📋分享我订阅的一些 Blog 和 Newsletter，通过 Github Actions，每天自动同步我 Feedly 上的订阅源，✅ 代表能正常订阅，❌ 代表暂无法订阅（对于无法订阅的 feed，支持 Telegram Bot、Email、Server酱等推送工具提醒更新）</sub>

---
## LampScript/qdaily_backup

> [!info]
> ⭐ 1,396 · N/A · 2019-08-30  
> [GitHub](https://github.com/LampScript/qdaily_backup) · [Website](https://www.qdaily.com/)  
> 
> 生成失败
> <sub>好奇心日报备份计划  本项目启动于好奇心日报因喝茶后被停更3个月，万一关门好文章无法再重新检索，所以在 github 上做备份，供大家参阅。</sub>

---
## pt-plugins/PT-Plugin-Plus

> [!info]
> ⭐ 7,884 · JavaScript · 2025-09-01  
> [GitHub](https://github.com/pt-plugins/PT-Plugin-Plus)  
> `#chrome-extension` `#edge-extension` `#firefox-addon` `#web-extension` 
> 生成失败
> <sub>PT 助手 Plus，为 Microsoft Edge、Google Chrome、Firefox 浏览器插件（Web Extensions），主要用于辅助下载 PT 站的种子。</sub>

---
## VirgilClyne/iRingo

> [!info]
> ⭐ 10,423 · N/A · 2024-11-20  
> [GitHub](https://github.com/VirgilClyne/iRingo) · [Website](https://NSRingo.github.io)  
> `#apple-maps` `#apple-news` `#apple-weather-app` `#geolocation` `#ios` `#ipados` `#lookup` `#macos` `#safari` `#siri` `#spotlight` 
> 生成失败
> <sub>解锁完整的 Apple功能和集成服务</sub>

---
## visiky/resume

> [!info]
> ⭐ 3,048 · TypeScript · 2023-08-30  
> [GitHub](https://github.com/visiky/resume) · [Website](https://visiky.github.io/resume/?template=template2&user=visiky)  
> `#gatsby` `#resume` `#resume-builder` `#resume-generator` 
> 生成失败
> <sub>🚀 在线简历生成器</sub>

---
## GrowingGit/GitHub-Chinese-Top-Charts

> [!info]
> ⭐ 106,684 · Java · 2024-10-12  
> [GitHub](https://github.com/GrowingGit/GitHub-Chinese-Top-Charts)  
> `#开源项目排行` `#中文开发者` `#GitHub榜单` 
> GitHub中文排行榜是一个专注于发现和推荐优质中文开源项目的平台，按编程语言和类别（软件/资料）设立总榜、增速榜和新秀榜，帮助用户无语言障碍地高效学习与交流，精准定位高质量中文项目。
> <sub>:cn: GitHub中文排行榜，各语言分设「软件 | 资料」榜单，精准定位中文好项目。各取所需，高效学习。</sub>

---
## DottChen/WeChatRead_NoteConvertor

> [!info]
> ⭐ 23 · Python · 2021-12-19  
> [GitHub](https://github.com/DottChen/WeChatRead_NoteConvertor)  
> 
> 生成失败
> <sub>微信读书导出的笔记格式太奇怪，所以写了个小脚本，可以将微信读书默认导出的笔记格式转化为标准markdown，方便同步到自己的笔记工具。</sub>

---
## Paladinfeng/MiaoProject

> [!info]
> ⭐ 2,681 · N/A · 2026-01-14  
> [GitHub](https://github.com/Paladinfeng/MiaoProject)  
> `#appletv` `#bilibili` `#miao` `#tvos` 
> 生成失败
> <sub>Miao Project Bug 反馈</sub>

---
## limbopro/Adblock4limbo

> [!info]
> ⭐ 4,360 · JavaScript · 2026-03-04  
> [GitHub](https://github.com/limbopro/Adblock4limbo) · [Website](https://limbopro.com/archives/12904.html)  
> `#adblock` `#adblocker` `#adguard` `#ios` `#javascript` `#limbopro` `#loon` `#nobyda` `#pornhub` `#quantumult` `#quantumultx` `#shadowrocket` `#stash` `#surge` `#surge4` `#tampermonkey` `#userscripts` 
> 生成失败
> <sub>毒奶去网页广告计划用户脚本 For Quantumult X & Surge & Shadowrocket & Loon & Stash & 油猴 ；1.导航 2.通过 JS/CSS 移除特定网站网页广告 —— 搜索引擎（Bing/Google）广告及内容农场结果清除/低端影视/欧乐影院/iyf爱壹帆/哔滴影视/Pornhub/Javbus/Supjav/Jable(M3U8)/MissAv/91porn/hitomi/紳士漫畫/禁漫天堂/等视频&ACG&小说&漫画网站上的弹窗广告&视频广告&Gif广告，沉浸式翻译/提取M3U8等，保持网页清爽干净无打扰！ P.S. 欢迎提交issue</sub>

---
## HarborLibrary/Political-Science

> [!info]
> ⭐ 2,206 · HTML · 2023-01-30  
> [GitHub](https://github.com/HarborLibrary/Political-Science)  
> 
> 生成失败
> <sub>政治</sub>

---
## saveweb/rss-list

> [!info]
> ⭐ 393 · N/A · 2024-09-28  
> [GitHub](https://github.com/saveweb/rss-list)  
> `#blog` `#china` `#podcast` `#rss` `#rss-reader` 
> 生成失败
> <sub>独立博客&播客全订阅计划！</sub>

---
## shibing624/pycorrector

> [!info]
> ⭐ 6,390 · Python · 2026-01-12  
> [GitHub](https://github.com/shibing624/pycorrector) · [Website](https://www.mulanai.com/product/corrector/)  
> `#csc` `#error-correction` `#error-detection` `#kenlm` `#macbert4csc` `#pycorrector` `#spelling-errors` `#t5` 
> 生成失败
> <sub>pycorrector is a toolkit for text error correction. 文本纠错，实现了Kenlm，T5，MacBERT，ChatGLM3，Qwen2.5等模型应用在纠错场景，开箱即用。</sub>

---
## DIYgod/RSSHub

> [!info]
> ⭐ 42,479 · TypeScript · 2026-03-09  
> [GitHub](https://github.com/DIYgod/RSSHub) · [Website](https://docs.rsshub.app)  
> `#bilibili` `#douban` `#dribbble` `#instagram` `#lofter` `#pixiv` `#rss` `#rsshub` `#spotify` `#telegram` `#tiktok` `#twitter` `#v2ex` `#wechat` `#weibo` `#ximalaya` `#youtube` `#zhihu` 
> 生成失败
> <sub>🧡 Everything is RSSible</sub>

---
## hepochen/FarBox

> [!info]
> ⭐ 170 · JavaScript · 2021-05-07  
> [GitHub](https://github.com/hepochen/FarBox)  
> 
> 生成失败
> <sub>FarBox 2.0 Core Framework</sub>

---
## Loyalsoldier/v2ray-rules-dat

> [!info]
> ⭐ 18,992 · N/A · 2026-03-08  
> [GitHub](https://github.com/Loyalsoldier/v2ray-rules-dat)  
> `#adblock` `#adguard` `#anticensorship` `#chinalist` `#dnsmasq` `#easylist` `#geoip` `#geosite` `#gfw` `#gfwlist` `#hysteria` `#ipv4` `#ipv6` `#mihomo` `#proxy` `#routing` `#trojan` `#trojan-go` `#v2ray` `#xray` 
> 生成失败
> <sub>🦄 🎃 👻 V2Ray 路由规则文件加强版，可代替 V2Ray 官方 geoip.dat 和 geosite.dat，适用于 V2Ray、Xray-core、mihomo(Clash-Meta)、hysteria、Trojan-Go 和 leaf。Enhanced edition of V2Ray rules dat files, applicable to V2Ray, Xray-core, mihomo(Clash-Meta), hysteria, Trojan-Go and leaf.</sub>

---
## zhaoolee/garss

> [!info]
> ⭐ 1,363 · Python · 2026-03-08  
> [GitHub](https://github.com/zhaoolee/garss) · [Website](https://zhaoolee.com/garss/)  
> `#rss` 
> 生成失败
> <sub>Github Actions采集RSS, 打造无广告内容优质的头版头条超赞宝藏页</sub>

---
## flomoapp/3rd-party-tools

> [!info]
> ⭐ 170 · N/A · 2020-12-17  
> [GitHub](https://github.com/flomoapp/3rd-party-tools)  
> 
> 生成失败

---
## alaskasquirrel/Chinese-Podcasts

> [!info]
> ⭐ 2,058 · N/A · 2023-10-06  
> [GitHub](https://github.com/alaskasquirrel/Chinese-Podcasts)  
> `#china` `#chinese` `#chinese-podcasts` `#podcast` 
> 生成失败
> <sub>播客 🎧 编程、设计、Vlog、音乐、访谈、博客...</sub>

---
## daliansky/OC-little

> [!info]
> ⭐ 1,640 · ASL · 2021-09-22  
> [GitHub](https://github.com/daliansky/OC-little)  
> `#bootloader` `#hackintosh` `#hotpatch` `#macos` `#opencore` 
> 生成失败
> <sub>ACPI Hotpatch Samples for the OpenCore Bootloader</sub>

---
## zlq4863947/proficient-tradingview

> [!info]
> ⭐ 173 · N/A · 2018-09-18  
> [GitHub](https://github.com/zlq4863947/proficient-tradingview)  
> 
> 生成失败
> <sub>24小时精通TradingView</sub>

---
## o1xhack/script

> [!info]
> ⭐ 31 · JavaScript · 2026-02-20  
> [GitHub](https://github.com/o1xhack/script)  
> 
> 生成失败
> <sub>o1xhack. Manage to use personally.</sub>

---
## 521xueweihan/HelloGitHub

> [!info]
> ⭐ 145,313 · Python · 2026-02-28  
> [GitHub](https://github.com/521xueweihan/HelloGitHub) · [Website](https://hellogithub.com)  
> `#开源推荐` `#入门项目` `#月刊` `#awesome` `#github` `#hellogithub` `#python` 
> HelloGitHub 是一个专注于分享 GitHub 上有趣、适合入门的开源项目的中文社区项目，每月28日发布月刊，涵盖开源项目、书籍、实战案例等，帮助开发者快速发现优质资源，激发对开源的兴趣与参与热情。
> <sub>:octocat: 分享 GitHub 上有趣、入门级的开源项目。Share interesting, entry-level open source projects on GitHub.</sub>

---
## google/eleventy-high-performance-blog

> [!info]
> ⭐ 4,055 · JavaScript · 2024-06-18  
> [GitHub](https://github.com/google/eleventy-high-performance-blog) · [Website](https://www.industrialempathy.com/posts/eleventy-high-performance-blog/)  
> `#11ty` `#11ty-template` `#eleventy` 
> 生成失败
> <sub>A high performance blog template for the 11ty static site generator.</sub>

---
## alirezamika/autoscraper

> [!info]
> ⭐ 7,109 · Python · 2025-06-09  
> [GitHub](https://github.com/alirezamika/autoscraper)  
> `#ai` `#artificial-intelligence` `#automation` `#crawler` `#machine-learning` `#python` `#scrape` `#scraper` `#scraping` `#web-scraping` `#webautomation` `#webscraping` 
> 生成失败
> <sub>A Smart, Automatic, Fast and Lightweight Web Scraper for Python</sub>

---
## yihong0618/running_page

> [!info]
> ⭐ 4,398 · Python · 2026-03-01  
> [GitHub](https://github.com/yihong0618/running_page) · [Website](https://running-page.vercel.app)  
> `#adidas` `#club-nike` `#data-analysis` `#data-visualization` `#garmin` `#garmin-watch` `#github-secret` `#gpx` `#mapbox` `#nike` `#strava` `#vercel` 
> 生成失败
> <sub>Make your own running home page</sub>

---
## AboutRSS/ALL-about-RSS

> [!info]
> ⭐ 5,711 · SCSS · 2026-03-04  
> [GitHub](https://github.com/AboutRSS/ALL-about-RSS) · [Website](https://rss.tips)  
> `#all-in-one` `#apps` `#atom-feed` `#atomfeed` `#community` `#curation` `#feed43` `#json-feed` `#jsonfeed` `#list` `#really-simple-sydication` `#rich-site-summary` `#rss` `#rss-aggregator` `#rss-feed` `#rss-reader` `#rssfeed` `#stuffs` `#telegram` `#telegram-channel` 
> 生成失败
> <sub>A list of RSS related stuff: tools, services, communities and tutorials, etc.</sub>

---
## athena-xcy/CS50-Study-Group

> [!info]
> ⭐ 239 · N/A · 2022-04-08  
> [GitHub](https://github.com/athena-xcy/CS50-Study-Group) · [Website](https://www.douban.com/group/cs50)  
> `#chinese-translation` `#cs50` `#edx-cs50` `#self-learning-community` `#study-group` `#subtitle-translation` `#volunteering` 
> 生成失败
> <sub>CS50 Study Group & Subtitle Translation Initiative</sub>

---
## headllines/hackernews-daily

> [!info]
> ⭐ 663 · JavaScript · 2024-01-19  
> [GitHub](https://github.com/headllines/hackernews-daily) · [Website](https://github.com/headllines/hackernews-daily/issues)  
> `#hackernews` `#headline-collectors` `#rss` 
> 生成失败
> <sub>Hacker News daily top 10 posts</sub>

---
## googlehosts/hosts

> [!info]
> ⭐ 20,709 · N/A · 2024-04-12  
> [GitHub](https://github.com/googlehosts/hosts)  
> `#hosts` 
> 生成失败
> <sub>镜像：https://scaffrey.coding.net/p/hosts/git / https://git.qvq.network/googlehosts/hosts</sub>

---
## iwestlin/gdshare

> [!info]
> ⭐ 489 · N/A · 2023-05-01  
> [GitHub](https://github.com/iwestlin/gdshare)  
> 
> 生成失败

---
## SuzukiHonoka/s905d-kernel-precompiled

> [!info]
> ⭐ 45 · Shell · 2026-02-27  
> [GitHub](https://github.com/SuzukiHonoka/s905d-kernel-precompiled)  
> 
> 生成失败
> <sub>由 Starx 预编译的 S905d 内核仓库。</sub>

---
## saturn-abhishek/awesome-github-profile-readme

> [!info]
> ⭐ 29,311 · N/A · 2026-03-03  
> [GitHub](https://github.com/saturn-abhishek/awesome-github-profile-readme) · [Website](https://bit.ly/awesome-gh)  
> `#awesome` `#awesome-list` `#github` `#github-profile-readme` `#github-readme` `#portfolio` `#profile-readme` 
> 生成失败
> <sub>😎 A curated list of awesome GitHub Profile which updates in real time</sub>

---
## gdtool/gd-utils-docker

> [!info]
> ⭐ 63 · Shell · 2020-09-15  
> [GitHub](https://github.com/gdtool/gd-utils-docker)  
> 
> 生成失败

---
## vitaminx/gd-utils

> [!info]
> ⭐ 159 · JavaScript · 2021-06-10  
> [GitHub](https://github.com/vitaminx/gd-utils)  
> 
> 生成失败
> <sub>Google Drive 百宝箱</sub>

---
## zvtvz/zvt

> [!info]
> ⭐ 4,009 · Python · 2026-03-07  
> [GitHub](https://github.com/zvtvz/zvt) · [Website](https://zvt.readthedocs.io/en/latest/)  
> `#algorithmic-trading` `#backtesting` `#cryptocurrency` `#fintech` `#fundamental-analysis` `#machine-learning` `#ml` `#python` `#quant` `#quantitative-finance` `#quantitative-trading` `#stock` `#stock-market` `#technical-analysis` `#trading-bot` `#trading-platform` `#trading-strategies` `#zvt` 
> 生成失败
> <sub>modular quant framework.</sub>

---
## ivmm/Student-resources

> [!info]
> ⭐ 6,116 · N/A · 2023-09-16  
> [GitHub](https://github.com/ivmm/Student-resources)  
> `#education` 
> 生成失败
> <sub>本文介绍的是利用学生、教职工身份可以享受到的相关学生优惠、教育优惠或教师优惠的权益，但也希望各位享受权利的同时不要忘记自己的义务，不要售卖、转手自己的学生优惠、教育优惠的资格，使得其他同学无法受益。</sub>

---
## iptv-org/iptv

> [!info]
> ⭐ 112,616 · TypeScript · 2026-03-09  
> [GitHub](https://github.com/iptv-org/iptv) · [Website](https://iptv-org.github.io)  
> `#iptv` `#m3u` `#playlist` `#streams` `#tv` 
> 生成失败
> <sub>Collection of publicly available IPTV channels from all over the world</sub>

---
## cgkings/script-store

> [!info]
> ⭐ 223 · Shell · 2024-09-29  
> [GitHub](https://github.com/cgkings/script-store)  
> 
> 生成失败
> <sub>script-store：vps practical script collection</sub>

---
## foxofice/sub_share

> [!info]
> ⭐ 1,597 · C# · 2025-09-26  
> [GitHub](https://github.com/foxofice/sub_share)  
> 
> 生成失败
> <sub>字幕共享计划</sub>

---
## SunRelease/BaiduSVIP

> [!info]
> ⭐ 752 · N/A · 2020-09-11  
> [GitHub](https://github.com/SunRelease/BaiduSVIP)  
> `#baidupan` `#baidusvip` `#pansvip` `#svip` 
> 生成失败
> <sub>每日分享百度网盘SVIP会员账号|百度云会员|百度网盘云账号,不定期更新,欢迎使用</sub>

---
## AlanDecode/Blog-With-GitHub-Boilerplate

> [!info]
> ⭐ 141 · Python · 2024-06-11  
> [GitHub](https://github.com/AlanDecode/Blog-With-GitHub-Boilerplate) · [Website](https://blog.imalan.cn/archives/blog-with-github/)  
> `#blog` `#github-pages` `#maverick` `#static-blog` 
> 生成失败
> <sub>这是博客文章《完全用 GitHub 写博客》所提出流程的示例仓库。</sub>

---
## awesome-selfhosted/awesome-selfhosted

> [!info]
> ⭐ 278,746 · N/A · 2026-03-08  
> [GitHub](https://github.com/awesome-selfhosted/awesome-selfhosted) · [Website](https://awesome-selfhosted.net/)  
> `#awesome` `#awesome-list` `#cloud` `#free-software` `#hosting` `#privacy` `#self-hosted` `#selfhosted` 
> 生成失败
> <sub>A list of Free Software network services and web applications which can be hosted on your own servers</sub>

---
## metowolf/vCards

> [!info]
> ⭐ 6,158 · JavaScript · 2026-03-02  
> [GitHub](https://github.com/metowolf/vCards)  
> `#china` `#ios` `#macos` `#vcards` 
> 生成失败
> <sub>📡️ vCards 中国黄页 - 优化 iOS/Android 来电、信息界面体验</sub>

---
## AktasC/Hackintosh-Elitebook-Broadwell

> [!info]
> ⭐ 18 · HTML · 2020-02-17  
> [GitHub](https://github.com/AktasC/Hackintosh-Elitebook-Broadwell) · [Website](https://aktasc.github.io/Hackintosh-Elitebook-Broadwell/#/)  
> 
> 生成失败
> <sub>WIP - Simple guide for a basic but solid Hackintosh on HP's Broadwell i7 Elitebook 840 G2</sub>

---
## sqlsec/clover

> [!info]
> ⭐ 1,348 · Rich Text Format · 2020-04-12  
> [GitHub](https://github.com/sqlsec/clover)  
> `#clover-efi` `#hackintosh` 
> 生成失败
> <sub>Share some hackintosh Clover files 分享一些黑苹果clover配置文件</sub>

---
## yisainan/web-interview

> [!info]
> ⭐ 2,746 · JavaScript · 2024-08-13  
> [GitHub](https://github.com/yisainan/web-interview)  
> `#css` `#html` `#interview` `#interview-questions` `#javascript` `#javascript-questions` `#vue` `#web-interview` 
> 生成失败
> <sub>我是齐丶先丶森，收集整理全网面试题及面试技巧，旨在帮助前端工程师们找到一份好工作！更多详见公众号「前端面试秘籍」</sub>

---
## typlog/china-indie-podcasts

> [!info]
> ⭐ 872 · Nunjucks · 2023-11-02  
> [GitHub](https://github.com/typlog/china-indie-podcasts) · [Website](https://typlog.com/podlist/)  
> 
> 生成失败
> <sub>发现与推荐高质量的中文独立播客</sub>

---
## mewcoder/UsefulWebSites

> [!info]
> ⭐ 223 · HTML · 2019-08-03  
> [GitHub](https://github.com/mewcoder/UsefulWebSites) · [Website](https://mewcoder.github.io/UsefulWebSites)  
> 
> 生成失败
> <sub>实用网站整理</sub>

---
## nailperry-zd/The-Economist

> [!info]
> ⭐ 3,790 · N/A · 2023-06-23  
> [GitHub](https://github.com/nailperry-zd/The-Economist)  
> `#economist` `#english` `#newspapers` 
> 生成失败
> <sub>The Economist 经济学人，持续更新</sub>

---
## GitHubDaily/GitHubDaily

> [!info]
> ⭐ 45,519 · N/A · 2025-12-31  
> [GitHub](https://github.com/GitHubDaily/GitHubDaily) · [Website](https://githubdaily.com)  
> `#ai` `#algorithms-and-data-structures` `#backend` `#developer-tools` `#development` `#frontend` `#github` `#java` `#javascript` `#kubernetes` `#linux` `#markdown` `#open-source` `#python` `#tutorials` `#web` 
> 生成失败
> <sub>坚持分享 GitHub 上高质量、有趣实用的开源技术教程、开发者工具、编程网站、技术资讯。A list cool, interesting projects of GitHub.</sub>

---
## yuanxin69/m3u

> [!info]
> ⭐ 609 · N/A · 2025-12-12  
> [GitHub](https://github.com/yuanxin69/m3u)  
> 
> 生成失败
> <sub>kodi list</sub>

---
